...
async function syncToGoogleCalendar(slotData, contactData) {
  ...
  if (!s.gcalKey) return; // no credentials – skip silently
  ...
  // Sync in beide Kalender (Kalender 1 + Kalender 2), je mit eigenem Key
  const calendars = [
    { id: s.gcalId,  key: s.gcalKey  },
    { id: s.gcalId2, key: s.gcalKey2 },
  ].filter(c => c.id && c.key);
  ...
  for (const cal of calendars) {
    const calId = cal.id;
    const url = `https://www.googleapis.com/calendar/v3/calendars/${encodeURIComponent(calId)}/events?key=${cal.key}`;
    try {
      ...
      if (res.ok) {
        ...
        console.warn(`Google Calendar API Fehler (${calId}):`, err);
      }
    } catch(e) {
      console.warn(`Google Calendar Sync fehlgeschlagen (${calId}):`, e.message);
    }
  }

  if (anySuccess) {
    toast(`✅ Termin in ${calendarIds.length > 1 ? 'beiden Google Kalendern' : 'Google Kalender'} eingetragen!`);
  } else {
...
function renderCalendar() {
  ...
  const slotsByDay = days.map(d => callSlots.filter(sl => {
    ...
    // Show slot on every day it spans
    const dStart = new Date(d); dStart.setHours(0,0,0,0);
    const dEnd = new Date(d); dEnd.setHours(23,59,59,999);
    return slStart <= dEnd && slEnd >= dStart;
  }));

  // Separate all-day (date-only) from timed events
  ...
  // Timed events: show on every day they span
          ...
          const dayCells = days.map((d, i) => {
            ...
            ">
              ...
              ${gcalHere.map(e => {
                ...
                // For multi-day events: clip start/end to this day's boundaries
                const dayStart = new Date(d); dayStart.setHours(globalStart/60|0, globalStart%60, 0, 0);
                const dayEnd = new Date(d); dayEnd.setHours(globalEnd/60|0, globalEnd%60, 0, 0);
                const clippedStart = eDate < dayStart ? dayStart : eDate;
                const clippedEnd = eEndDate > dayEnd ? dayEnd : eEndDate;

                const startMin = clippedStart.getHours() * 60 + clippedStart.getMinutes();
                const endMin = clippedEnd.getHours() * 60 + clippedEnd.getMinutes();
                const durMin = Math.max(30, endMin - startMin);

                // Only render in the slot where this event starts (for this day)
                if (startMin < slotMin || startMin >= slotMin + 30) return '';
                ...
                const isContinuation = eDate.toDateString() !== d.toDateString();
                const continuesNext = eEndDate.toDateString() !== d.toDateString() && eEndDate > dayEnd;

                const timeLabel = isContinuation
                  ? `← ${eDate.toLocaleDateString('de-DE',{day:'2-digit',month:'2-digit'})}`
                  : eDate.toLocaleTimeString('de-DE',{hour:'2-digit',minute:'2-digit'});
                const endLabel = continuesNext ? ` →` : '';
                ...
                <div title="${e.title} (${e.calendarLabel})" onclick="window.open('${e.htmlLink||''}','_blank')" style="
                  ...
                  border-radius:${isContinuation ? '0 6px 6px 0' : continuesNext ? '6px 6px 0 0' : '6px'};
                  padding:3px 6px;
                  cursor:pointer;
                  overflow:hidden;
                  z-index:1;
                ...
                " onmouseover="this.style.opacity='.8'" onmouseout="this.style.opacity='1'">
                  <div style="font-size:10px;font-weight:600;color:${gcalColor};white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">
                    ${timeLabel}${endLabel} · ${e.calendarLabel}
                  </div>
