...
function renderCalendar() {
  ...
  const gcalByDay = days.map(d => googleCalendarEvents.filter(e => {
    if (!e.datetime || !e.datetime.includes('T')) return false;
    if (allSlotGcalIds.has(e.id)) return false; // bereits als Slot sichtbar
  ...
  // ── GCal-Events in Zeitbereich einbeziehen (Termine außerhalb Geschäftszeiten sichtbar) ──
  days.forEach((d, i) => {
...
window.confirmPrivateSlot = async function(slotId) {
  ...
  document.getElementById('pn_confirm_btn').onclick = async () => {
    ...
    // ── Kontakt anlegen beim Bestätigen (falls noch nicht vorhanden) ──
