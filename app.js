...
function kaRenderKaChats() {
  ...
  const cards = chats.map((chat, idx) => {
    ...
    const msgsHtml = msgs.map(m => {
      ...
      return `<div style="display:flex;gap:10px;margin-bottom:8px;align-items:flex-start;${isMe ? 'flex-direction:row-reverse;' : ''}">
        <span style="font-size:11px;font-weight:600;color:var(--text3);white-space:nowrap;padding-top:3px;${isMe ? 'text-align:left;' : 'text-align:right;min-width:52px;'}">
          ${isMe ? 'Ich' : 'Kontakt'}
        </span>
        <span style="font-size:13px;line-height:1.5;border-radius:8px;padding:6px 10px;max-width:80%;
          ${isMe
            ? 'background:rgba(139,92,246,0.15);color:var(--text2);margin-left:auto;'
