...
async function loadGoogleCalendarEvents() {
  ...
  if (calendars.length === 0) {
    console.warn('Kein Google Kalender konfiguriert (API-Key oder Kalender-ID fehlt).');
    return;
  }
  ...
  for (const cal of calendars) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        console.warn(`Google Calendar Import Fehler (${cal.label}):`, reason);
        errors.push(`${cal.label}: ${reason}`);
      }
    } catch(e) {
      console.warn(`Google Calendar Import fehlgeschlagen (${cal.label}):`, e.message);
      errors.push(`${cal.label}: ${e.message}`);
    }
  }
  ...
  // Return status info for the caller
  return { count: googleCalendarEvents.length, errors };
}
...
{signatur}`;
  ...
  document.getElementById('loadGcalBtn')?.addEventListener('click', async () => {
    ...
    const count = result?.count ?? googleCalendarEvents.length;
    const errors = result?.errors ?? [];
    if (count > 0) {
      toast(`✅ ${count} Termine aus Google Kalender geladen!`, 'success');
    } else if (errors.length > 0) {
      toast(`Fehler beim Laden: ${errors[0]}`, 'error');
      console.error('Google Calendar Fehler:', errors);
    } else {
      toast('Keine Termine im gewählten Zeitraum gefunden.', 'error');
    }
  });
}
