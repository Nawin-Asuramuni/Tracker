...
let kaSocialActiveChannels = new Set(); // Checkboxes: welche Kanäle aufgeklappt
// Multi-Select State
  ...
  const kanalBlocks = KA_SOCIAL_CHANNELS.map(k => {
    ...
    return `
      <div style="background:var(--surface, var(--card-bg));border:1px solid ${checked ? col.c + '66' : 'var(--border, var(--border-color))'};
        ...
        <label style="display:flex;align-items:center;gap:14px;padding:14px 16px;cursor:pointer;user-select:none;"
          onclick="kaSocialToggleChannel('${k}');return false;">
          ...
          <i class="ti ${checked ? 'ti-chevron-up' : 'ti-chevron-down'}" style="font-size:16px;color:var(--text3, var(--text-muted));flex-shrink:0;"></i>
