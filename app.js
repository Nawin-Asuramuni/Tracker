...
const pages = {
  ...
  socialmedia: { title: 'Social Media', render: openSocialMediaPanel },
};
window.pages = pages; // Expose for kleinanzeigen.js (ES-Module scope fix)

function navigate(page) {
  currentPage = page;
  // Social Media öffnet ein Overlay-Panel, kein normales Page-Render
  if (page === 'socialmedia') {
...
function openSocialMediaPanel() {
  const panel = document.getElementById('socialmedia-panel-embed');
  if (!panel) return;
...
function closeSocialMediaPanel() {
  const panel = document.getElementById('socialmedia-panel-embed');
  if (!panel) return;
...
window.closeSocialMediaPanel = closeSocialMediaPanel;
