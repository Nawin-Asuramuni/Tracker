...
window.confirmPrivateSlot = async function(slotId) {
  ...
  document.getElementById('pn_confirm_btn').onclick = async () => {
    ...
    if (slot.telefon) {
      ...
      if (sendReminder) {
        const reminderMsg = `Hey ${slot.vorname} 👋\n\nNur kurz zur Erinnerung – heute steht unser Termin an:\n📅 ${datum}\n🕐 ${von} – ${bis} Uhr\n${slot.apptType ? '🎯 ' + slot.apptType + '\n' : ''}\nBis gleich! 🙌`;
        setTimeout(() => {
          if (slot.telefon) window.open(`https://wa.me/${phone}?text=${encodeURIComponent(reminderMsg)}`, '_blank');
        }, sendConfirm ? 1200 : 0);
      }

      if (sendNotify) {
        const notifyMsg = `Hallo ${slot.vorname},\n\nhier sind deine Termindetails:\n📅 ${datum}\n🕐 ${von} – ${bis} Uhr\n${slot.apptType ? '🎯 ' + slot.apptType + '\n' : ''}${ownerName ? '👤 ' + ownerName : ''}\n\nBei Fragen melde dich gerne. 😊`;
        const delay = (sendConfirm ? 1200 : 0) + (sendReminder ? 1200 : 0);
        setTimeout(() => {
          if (slot.telefon) window.open(`https://wa.me/${phone}?text=${encodeURIComponent(notifyMsg)}`, '_blank');
        }, delay);
      }
    }
  };
...
window.deletePrivateSlotFromCal = async function(slotId) {
  ...
  if (!confirm(`${status}privaten Termin für ${name} löschen?\n\nDer Termin wird aus dem System entfernt.`)) return;
  try {
    await deleteDoc(doc(db, 'callSlots', slotId));
    toast('Privater Termin gelöscht.');
  } catch {
    privateSlots = privateSlots.filter(s => s.id !== slotId);
    renderCalendar();
  ...
  document.getElementById('bn_confirm_btn').onclick = async () => {
    ...
    if (sendConfirm) {
      if (slot.email) {
        ...
        window.open(`mailto:${slot.email}?subject=${subject}&body=${body}`, '_blank');
    ...
    if (sendReminder && slot.telefon) {
      const phone = slot.telefon.replace(/[^0-9+]/g, '').replace(/^0/, '49');
      const reminderMsg = `Guten Morgen ${slot.vorname},\n\nnur kurz zur Erinnerung – heute haben Sie einen Termin bei uns:\n📅 ${datum}\n🕐 ${von} – ${bis} Uhr\n🎯 ${slot.apptType || 'Beratungsgespräch'}\n\nWir freuen uns auf Sie!\n${ownerName}`;
      setTimeout(() => window.open(`https://wa.me/${phone}?text=${encodeURIComponent(reminderMsg)}`, '_blank'), delay);
      delay += 1200;
    }

    if (sendNotify && slot.telefon) {
      const phone = slot.telefon.replace(/[^0-9+]/g, '').replace(/^0/, '49');
      const notifyMsg = `Hallo ${slot.vorname},\n\nhier sind Ihre Termindetails:\n📅 ${datum}\n🕐 ${von} – ${bis} Uhr\n🎯 ${slot.apptType || 'Beratungsgespräch'}\n👤 ${ownerName}\n\nBei Fragen stehen wir Ihnen gerne zur Verfügung.`;
      setTimeout(() => window.open(`https://wa.me/${phone}?text=${encodeURIComponent(notifyMsg)}`, '_blank'), delay);
    }
  };
};
