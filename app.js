...
// ── Chats-Tab (Kleinanzeigen) ──────────────────────────────────
function kaRenderKaChats() {
  const SK = 'ka_social_chats';
  let chats = [];
  try { chats = JSON.parse(localStorage.getItem(SK) || '[]'); } catch {}

  const ICONS = {
    instagram:'📸', tiktok:'🎵', linkedin:'💼', twitter:'🐦',
    whatsapp:'💬', bumble:'🐝', snapchat:'👻', tellonym:'💌', unknown:'💭'
  };
  ...
  if (!chats.length) {
    return `<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;padding:48px 24px;text-align:center;gap:12px;">
      <i class="ti ti-message-off" style="font-size:40px;color:var(--text3,var(--text-muted));"></i>
      <p style="color:var(--text2,var(--text-muted));font-size:14px;max-width:320px;">
        Noch keine Chats gespeichert.<br>Die Browser-Extension trägt Chats von Instagram, TikTok & Co. hier automatisch ein.
      </p>
    </div>`;
  }

  const cards = chats.map((chat, idx) => {
    const icon = ICONS[chat.platform?.toLowerCase()] || '💭';
    const name = (chat.contactName || chat.url || 'Unbekannt').replace(/</g,'&lt;');
    const msgs = (chat.messages || []).slice(-20);
    const msgsHtml = msgs.map(m => `
      <div style="display:flex;gap:10px;margin-bottom:8px;align-items:flex-start;">
        <span style="font-size:11px;font-weight:600;color:var(--text3,var(--text-muted));white-space:nowrap;min-width:52px;padding-top:2px;text-align:right;">
          ${m.sender === 'ich' ? 'Ich' : 'Kontakt'}
        </span>
        <span style="font-size:13px;color:${m.sender === 'ich' ? 'var(--text2,#94a3b8)' : 'var(--text,#f1f5f9)'};line-height:1.5;background:${m.sender === 'ich' ? 'rgba(255,255,255,0.04)' : 'rgba(139,92,246,0.08)'};border-radius:8px;padding:6px 10px;flex:1;">
          ${m.text.replace(/</g,'&lt;')}
        </span>
      </div>`).join('');

    return `
      <div style="background:var(--bg2,var(--card-bg));border:1px solid var(--border,var(--border-color));border-radius:12px;overflow:hidden;margin-bottom:12px;">
        <div style="display:flex;align-items:center;gap:10px;padding:14px 16px;border-bottom:1px solid var(--border,var(--border-color));">
          <span style="font-size:22px;">${icon}</span>
          <div style="flex:1;min-width:0;">
            <div style="font-weight:700;font-size:14px;color:var(--text,var(--text-color));white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${name}</div>
            <div style="font-size:11px;color:var(--text3,var(--text-muted));margin-top:2px;">${chat.platform || ''} · ${timeAgo(chat.savedAt)}</div>
          </div>
          <div style="display:flex;align-items:center;gap:8px;flex-shrink:0;">
            ${chat.url ? `<a href="${chat.url}" target="_blank" style="font-size:12px;color:var(--purple,#a78bfa);text-decoration:none;border:1px solid rgba(139,92,246,0.3);border-radius:7px;padding:4px 9px;">Öffnen ↗</a>` : ''}
            <button onclick="window.kaDeleteKaChat(${idx})" style="background:none;border:none;color:var(--text3,var(--text-muted));cursor:pointer;font-size:16px;padding:4px 7px;border-radius:6px;line-height:1;" title="Chat löschen">✕</button>
          </div>
        </div>
        <div style="padding:14px 16px;max-height:240px;overflow-y:auto;">${msgsHtml || '<p style="font-size:13px;color:var(--text3);">Keine Nachrichten.</p>'}</div>
      </div>`;
  }).join('');

  return `
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;">
      <span style="font-size:13px;color:var(--text2,var(--text-muted));">${chats.length} Chat${chats.length !== 1 ? 's' : ''} gespeichert</span>
    </div>
    ${cards}`;
}
...
window.kaDeleteKaChat = function(idx) {
  try {
    const chats = JSON.parse(localStorage.getItem('ka_social_chats') || '[]');
    chats.splice(idx, 1);
    localStorage.setItem('ka_social_chats', JSON.stringify(chats));
...
window.renderKleinanzeigen = function () {
  ...
  const chatCount = (() => { try { return JSON.parse(localStorage.getItem('ka_social_chats') || '[]').length; } catch { return 0; } })();
  const tabs = [
    ...
    { id:'chats',         label:`<i class="ti ti-message-circle"></i> Chats${chatCount > 0 ? ` <span style="background:rgba(139,92,246,0.15);color:var(--purple,#a78bfa);border-radius:20px;padding:1px 7px;font-size:11px;">${chatCount}</span>` : ''}` },
  ];
  const tabContents = {
