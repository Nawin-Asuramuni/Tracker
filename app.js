...
window.renderContacts = function(filter = '', statusFilter = '', typeFilter = '', catFilter = '') {
  ...
  const filtered = contacts.filter(c => {
    // Suche auch in Kategorie-Label und Quelle
  ...
  document.getElementById('content').innerHTML = `
    ...
    <div style="display:flex;gap:6px;margin-bottom:14px;flex-wrap:wrap;align-items:center;">
      <button onclick="renderContacts(document.getElementById('searchInput')?.value||'',document.getElementById('statusFilter')?.value||'','','')"
        ...
        cursor:pointer;font-weight:${!catFilter?'600':'400'};transition:all .15s;">
        Alle <span style="font-size:11px;opacity:.7;">(${contacts.length})</span>
      </button>
      ${Object.entries({
        ...
        <button onclick="renderContacts(document.getElementById('searchInput')?.value||'',document.getElementById('statusFilter')?.value||'','','${key}')"
          style="${tabStyle(key)}">
          <i class="ti ${catIcons[key]}" style="font-size:12px;"></i> ${lbl} <span style="font-size:11px;opacity:.7;">(${catCounts[key]})</span>
        </button>
      `).join('')}
      ${catFilter ? `<button onclick="renderContacts(document.getElementById('searchInput')?.value||'',document.getElementById('statusFilter')?.value||'','','')"
        ...
        <i class="ti ti-x"></i> Aufheben</button>` : ''}
    </div>
