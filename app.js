...
// Ein einfacher API-Key kann keine Events schreiben – dafür brauchen
// wir OAuth 2.0. Der Token wird im localStorage gecacht.
// ============================================================
let _gcalOAuthToken = null;

async function getGCalOAuthToken(calendarEmail) {
  // Return cached token if still valid (5 min buffer)
  const cached = JSON.parse(localStorage.getItem('gcalOAuthToken_' + calendarEmail) || 'null');
  if (cached && cached.expires > Date.now() + 300000) {
    _gcalOAuthToken = cached.token;
    return cached.token;
  }
    ...
    const initClient = () => {
      const tokenClient = google.accounts.oauth2.initTokenClient({
        ...
        callback: (resp) => {
          ...
          localStorage.setItem('gcalOAuthToken_' + calendarEmail, JSON.stringify({ token: resp.access_token, expires: expiry }));
...
async function syncToGoogleCalendar(slotData, contactData) {
  ...
  const event = {
    ...
    description: [
      slotData.note || '',
      contactData?.telefon ? `Tel: ${contactData.telefon}` : '',
    ].filter(Boolean).join('\n'),
  ...
  for (const cal of calendars) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        console.warn(`Google Calendar Fehler (${cal.id}):`, err);
        // Token might be expired – clear it
        if (res.status === 401) localStorage.removeItem('gcalOAuthToken_' + cal.email);
        needsFallback = true;
      }
    } catch(e) {
      console.warn(`Google Calendar Sync fehlgeschlagen (${cal.id}):`, e.message);
      needsFallback = true;
    }
  }

  if (successCount > 0) {
    toast(`✅ Termin in ${successCount > 1 ? 'beiden Google Kalendern' : 'Google Kalender'} eingetragen!`, 'success');
    setTimeout(() => loadGoogleCalendarEvents(), 1500);
  } else if (needsFallback) {
    // Fallback: open Google Calendar web UI
    openGoogleCalendar(
...
function renderCalendar() {
  ...
  const gcalByDay = days.map(d => googleCalendarEvents.filter(e => {
    ...
    return new Date(e.datetime).toDateString() === d.toDateString();
  }));
          ...
          const dayCells = days.map((d, i) => {
            ...
            ">
              ...
              ${gcalHere.map(e => {
                const eDate = new Date(e.datetime);
                const eMin = eDate.getHours() * 60 + eDate.getMinutes();
                const eEndDate = e.end ? new Date(e.end) : new Date(eDate.getTime() + 30*60000);
                const durMin = Math.max(30, (eEndDate - eDate) / 60000);
                const slotCount = Math.ceil(durMin / 30);
                const heightPx = slotCount * ROW_HEIGHT - 4;
                const offsetPx = ((eMin - slotMin) / 30) * ROW_HEIGHT;
                ...
                " onmouseover="this.style.opacity='.8'" onmouseout="this.style.opacity='1'">
                  <div style="font-size:10px;font-weight:600;color:${gcalColor};white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">
                    ${eDate.toLocaleTimeString('de-DE',{hour:'2-digit',minute:'2-digit'})} · ${e.calendarLabel}
                  </div>
...
{signatur}`;
  ...
  document.getElementById('content').innerHTML = `
    ...
    <div class="settings-section">
      ...
      <!-- OAuth Client ID für das Schreiben von Terminen -->
      <div style="background:var(--bg3);border:1px solid var(--border);border-radius:10px;padding:14px;margin-top:12px;">
        ...
        <p style="font-size:12px;color:var(--text2);margin-bottom:10px;">
          Damit Termine automatisch in Google Kalender eingetragen werden, brauchst du eine OAuth Client-ID.<br>
          <a href="https://console.cloud.google.com/apis/credentials" target="_blank" style="color:var(--accent);">console.cloud.google.com</a>
          → Anmeldedaten → OAuth 2.0-Client-IDs → Web-App → Autorisierte JS-Ursprünge: <code style="font-size:11px;background:var(--bg2);padding:1px 4px;border-radius:3px;">https://nawin-asuramuni.github.io</code>
        </p>
        <div class="field full"><label>OAuth Client-ID</label>
          <input type="text" id="s_gcalOAuthClientId" placeholder="xxxx.apps.googleusercontent.com" value="${s.gcalOAuthClientId||''}" />
        </div>
      </div>
