...
function _csGetDayConfig(jsDay) {
  const cfg = JSON.parse(localStorage.getItem('crmSettings') || '{}');
  return (cfg.dayConfigs || {})[jsDay] || null;
}
...
async function _csLoadTakenSlots() {
  try {
    ...
    const to   = new Date(Date.now() + 90*86400000).toISOString().slice(0,16);
    _csTakenSlots = callSlots.filter(s => s.datetime >= from && s.datetime <= to).map(s => s.datetime);
