...
function kaRenderKaChats() {
  ...
  const cards = chats.map((chat, idx) => {
    ...
    const msgsHtml = msgs.map(m => `
      <div style="display:flex;gap:10px;margin-bottom:8px;align-items:flex-start;">
        <span style="font-size:11px;font-weight:600;color:var(--text3);white-space:nowrap;min-width:52px;padding-top:3px;text-align:right;">
          ${m.sender === 'ich' ? 'Ich' : 'Kontakt'}
        </span>
        <span style="font-size:13px;color:${m.sender === 'ich' ? 'var(--text2)' : 'var(--text)'};line-height:1.5;background:${m.sender === 'ich' ? 'rgba(255,255,255,0.04)' : 'rgba(139,92,246,0.08)'};border-radius:8px;padding:6px 10px;flex:1;">
          ${m.text.replace(/</g,'&lt;')}
        </span>
      </div>`).join('');
      ...
      <div style="background:var(--bg2);border:1px solid var(--border);border-radius:12px;overflow:hidden;margin-bottom:12px;">
        <div style="display:flex;align-items:center;gap:10px;padding:14px 16px;border-bottom:1px solid var(--border);">
          ...
          <div style="flex:1;min-width:0;">
            ...
            <div style="font-size:11px;color:var(--text3);margin-top:2px;">${chat.platform || ''} · ${timeAgo(chat.savedAt)}</div>
          </div>
          <div style="display:flex;align-items:center;gap:8px;flex-shrink:0;">
            ${chat.url ? `<a href="${chat.url}" target="_blank" style="font-size:12px;color:var(--purple);text-decoration:none;border:1px solid rgba(139,92,246,0.3);border-radius:7px;padding:4px 9px;">Öffnen ↗</a>` : ''}
            <button onclick="window.kaDeleteKaChat(${idx})" style="background:none;border:none;color:var(--text3);cursor:pointer;font-size:16px;padding:4px 7px;border-radius:6px;line-height:1;" title="Chat löschen">✕</button>
          </div>
        </div>
        <div style="padding:14px 16px;max-height:240px;overflow-y:auto;">
  ...
  return `
    <div style="margin-bottom:16px;font-size:13px;color:var(--text2);">${chats.length} Chat${chats.length !== 1 ? 's' : ''} gespeichert</div>
    ${cards}`;
}
