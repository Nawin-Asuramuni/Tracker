...
function renderCalendar() {
  ...
  document.getElementById('content').innerHTML = `
    ...
    <!-- Calendar grid -->
    <div class="week-wrap" style="overflow-x:auto;">
      <div style="display:grid;grid-template-columns:repeat(7,minmax(120px,1fr));gap:8px;min-width:700px;">
        ${days.map((d, i) => {
          ...
          return `
          <div style="
            background:${isUnavailable||isBlocked ? 'var(--bg)' : isToday ? 'rgba(59,130,246,0.07)' : 'var(--surface)'};
            border:1.5px solid ${isToday ? 'rgba(59,130,246,0.5)' : 'var(--border)'};
            border-radius:14px;min-height:280px;padding-bottom:8px;
            ${isUnavailable||isBlocked ? 'opacity:0.45;' : ''}
            display:flex;flex-direction:column;overflow:hidden;
          ">
            <!-- Day header -->
            <div style="padding:12px 12px 8px;border-bottom:1px solid var(--border);${isToday?'background:rgba(59,130,246,0.1);':''}">
              <div style="font-size:11px;text-transform:uppercase;letter-spacing:.08em;font-weight:600;color:${isToday?'var(--accent)':'var(--text3)'};">${names[i]}</div>
              <div style="font-size:${isToday?'26':'22'}px;font-weight:800;color:${isToday?'var(--accent)':'var(--text)'};line-height:1.1;margin-top:2px;">${d.getDate()}</div>
              <div style="font-size:11px;color:var(--text3);margin-top:2px;">
                ${d.toLocaleDateString('de-DE', { month:'short' })}
                ${isBlocked ? ' · <span style="color:var(--red);">🚫 Blockiert</span>' : isUnavailable ? ' · Nicht verfügbar' : timeLabel ? ` · ${timeLabel}` : ''}
              </div>
              ${breakLabel && !isUnavailable && !isBlocked ? `<div style="font-size:10px;color:var(--text3);margin-top:1px;">${breakLabel}</div>` : ''}
            </div>

            <!-- Slots -->
            <div style="padding:8px;display:flex;flex-direction:column;gap:5px;flex:1;">
              ${daySlots.map(sl => {
                ...
                <div onclick="showContact('${sl.contactId||''}')" style="
                  padding:8px 10px;border-radius:9px;cursor:pointer;transition:opacity .15s;
                  background:${color}${bgAlpha};
                  border-left:4px solid ${color};
                " title="${sl.note || name}">
                  <div style="font-size:12px;font-weight:700;color:${color};white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${time}${sl.apptType ? ' · ' + sl.apptType : ''}</div>
                  <div style="font-size:13px;color:var(--text);margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;font-weight:500;">${name}</div>
                  ${sl.apptDuration ? `<div style="font-size:11px;color:var(--text3);margin-top:2px;">⏱ ${sl.apptDuration} Min.</div>` : ''}
                </div>`;
              }).join('')}

              ${!isUnavailable && !isBlocked ? `
              <button onclick="openCallSlotModalForDate('${d.toISOString()}')"
                style="width:100%;background:none;border:1.5px dashed var(--border2);color:var(--text3);border-radius:9px;
                padding:8px;font-size:13px;cursor:pointer;margin-top:2px;transition:all .15s;"
                onmouseover="this.style.borderColor='var(--accent)';this.style.color='var(--accent)'"
                onmouseout="this.style.borderColor='var(--border2)';this.style.color='var(--text3)'">
                <i class="ti ti-plus" style="font-size:14px;"></i> Termin
              </button>` : ''}
            </div>
          </div>`;
        }).join('')}
