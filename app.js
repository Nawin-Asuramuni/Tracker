...
// ============================================================
// E-MAIL TERMINBESTÄTIGUNG – sofort beim Eintragen
// ============================================================
function sendConfirmationEmail(contact, slotData) {
  const s = JSON.parse(localStorage.getItem('crmSettings') || '{}');
  if (!contact?.email) {
    toast('Kein E-Mail für diesen Kontakt hinterlegt.', 'error');
    return;
  }
  ...
  const subject = (s.confirmSubject || 'Terminbestätigung – {datum}')
    .replace('{datum}', datum)
    .replace('{uhrzeit}', uhrzeit);

  const sigText = s.emailSig || '';
  const body = (s.emailConfirmText ||
`Hallo {vorname},
...
{signatur}`)
    ...
    .replace(/{signatur}/g, sigText);

  const mailtoUrl = `mailto:${contact.email}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
  window.open(mailtoUrl, '_blank');
  toast(`📧 E-Mail für ${contact.vorname} vorbereitet – bitte in deinem Mailclient abschicken.`, 'success');
}
...
async function syncToGoogleCalendar(slotData, contactData) {
  ...
  if (successCount > 0) {
    toast(`✅ In ${successCount > 1 ? 'beiden Google Kalendern' : 'Google Kalender'} eingetragen!`, 'success');
          ...
          const dayCells = days.map((d, i) => {
            ...
            ">
              ...
              ${gcalHere.map(e => {
                ...
                return `
                <div title="${e.title} (${e.calendarLabel})" onclick="window.open('${e.htmlLink||''}','_blank')" style="
                  position:absolute;
                  top:${offsetPx + 2}px;
                  left:3px;right:3px;
                  height:${heightPx}px;
...
document.getElementById('saveCallSlot').addEventListener('click', async () => {
  ...
  try {
    ...
    // Google Calendar Sync (falls OAuth-ClientId hinterlegt)
  ...
  // Terminbestätigung per E-Mail (sofort beim Eintragen)
  if (data.sendConfirm && contactForSync?.email) {
    sendConfirmationEmail(contactForSync, data);
  } else if (data.sendConfirm && !contactForSync?.email) {
    toast('⚠️ Keine E-Mail beim Kontakt hinterlegt – Bestätigung konnte nicht geöffnet werden.', 'error');
  }
...
{signatur}`;
  ...
  document.getElementById('content').innerHTML = `
    ...
    <!-- ── TERMINBESTÄTIGUNG AUTOMATISIERUNG ── -->
    <div class="settings-section">
      <h3>⚡ Automatisierung – Terminbestätigung & Kalender</h3>
      <p style="font-size:13px;color:var(--text2);margin-bottom:16px;">
        Steuere hier, was automatisch passiert, sobald du einen Termin einträgst – wie bei Calendly.
      </p>
      ...
      <div style="display:flex;flex-direction:column;gap:12px;">
        ...
        <div style="background:var(--bg3);border:1px solid var(--border);border-radius:10px;padding:14px 16px;">
          <div style="display:flex;align-items:flex-start;gap:12px;">
            ...
            <div style="flex:1;">
              <div style="font-weight:600;font-size:14px;margin-bottom:4px;">Bestätigungs-E-Mail sofort versenden</div>
              <div style="font-size:13px;color:var(--text2);margin-bottom:10px;">
                Sobald du einen Termin einträgst, wird dein Mailclient mit vorausgefüllter Bestätigung geöffnet – du musst nur noch abschicken.
              </div>
              <label style="display:flex;align-items:center;gap:8px;cursor:pointer;">
                <input type="checkbox" id="s_autoConfirm" ${s.autoConfirm?'checked':''} />
                <span style="font-size:13px;">Standardmäßig Bestätigung vorbereiten</span>
              </label>
            </div>
          </div>
        </div>

        <!-- Google Kalender Automatik -->
        <div style="background:var(--bg3);border:1px solid var(--border);border-radius:10px;padding:14px 16px;">
          <div style="display:flex;align-items:flex-start;gap:12px;">
            <div style="font-size:22px;margin-top:2px;">📅</div>
            <div style="flex:1;">
              <div style="font-weight:600;font-size:14px;margin-bottom:4px;">Termin automatisch in Google Kalender eintragen</div>
              <div style="font-size:13px;color:var(--text2);margin-bottom:10px;">
                Wie Calendly – jeder neue Termin wird sofort in deinen Google Kalender eingetragen. Dafür wird die OAuth Client-ID unten benötigt.
              </div>
              <div style="font-size:13px;color:${s.gcalOAuthClientId ? 'var(--green)' : 'var(--amber)'};">
                <i class="ti ti-${s.gcalOAuthClientId ? 'check-circle' : 'alert-circle'}"></i>
                ${s.gcalOAuthClientId
                  ? 'OAuth aktiv – Termine werden automatisch eingetragen'
                  : 'OAuth Client-ID fehlt – bitte unten im Google Kalender Abschnitt eintragen'}
              </div>
              ${!s.gcalOAuthClientId ? `
              <button class="btn-ghost" style="margin-top:8px;font-size:12px;" onclick="document.querySelector('#content .settings-section:last-of-type').scrollIntoView({behavior:'smooth'})">
                <i class="ti ti-arrow-down"></i> Zu Google Kalender OAuth springen
              </button>` : ''}
            </div>
          </div>
        </div>
        ...
        <div style="background:var(--bg3);border:1px solid var(--border);border-radius:10px;padding:14px 16px;">
          <div style="display:flex;align-items:flex-start;gap:12px;">
            <div style="font-size:22px;margin-top:2px;">🔔</div>
            <div style="flex:1;">
              <div style="font-weight:600;font-size:14px;margin-bottom:4px;">Erinnerungs-E-Mail am Termintag</div>
              <div style="font-size:13px;color:var(--text2);margin-bottom:10px;">
                Standardmäßig beim Einplanen eines Termins die Erinnerungsoption vorauswählen.
              </div>
              <label style="display:flex;align-items:center;gap:8px;cursor:pointer;">
                <input type="checkbox" id="s_autoReminder" ${s.autoReminder?'checked':''} />
                <span style="font-size:13px;">Standardmäßig Erinnerung vorbereiten</span>
              </label>
            </div>
          </div>
        </div>
