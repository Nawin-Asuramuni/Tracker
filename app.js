...
function _csRenderCal() {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    const avail   = !isPast && _csIsDayAvail(date);
...
function _csRenderTimeSlots(y, m, d) {
  ...
  for (let t = startMin; t + dur <= endMin; t += 30) {
    ...
    const isoStr  = `${y}-${_csPad(m+1)}-${_csPad(d)}T${timeStr}`;
    const slotDate = new Date(y, m, d, slotH, slotM);
    if (slotDate <= nowDate) continue;
    const isTaken = _csTakenSlots.some(ts => ts.slice(0,16) === isoStr.slice(0,16));
  ...
  slots.forEach(sl => {
    ...
    btn.className = 'cs-time-slot' + (sl.isTaken ? ' taken' : '');
...
async function _csLoadTakenSlots() {
  try {
    ...
    callSlots.forEach(sl => {
      ...
      if (start > to) return;
      // Expand every 30-min block the appointment covers (handles multi-day too)
      const dur = sl.apptDuration || 30;
      const end  = new Date(start.getTime() + dur * 60000);
      const cur  = new Date(start);
      cur.setSeconds(0, 0);
      cur.setMinutes(Math.floor(cur.getMinutes() / 30) * 30);
      while (cur < end) {
        _csTakenSlots.push(
          cur.getFullYear() + '-' + _csPad(cur.getMonth()+1) + '-' + _csPad(cur.getDate()) +
          'T' + _csPad(cur.getHours()) + ':' + _csPad(cur.getMinutes())
        );
        cur.setMinutes(cur.getMinutes() + 30);
      }
    });
  } catch(e) { _csTakenSlots = []; }
}
