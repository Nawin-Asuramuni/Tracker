...
// ── Notizen-Overlay (wie Booking-Overlay, schließt zurück zum Tracker) ──
function openNotesOverlay() {
  document.getElementById('notes-overlay')?.remove();

  const overlay = document.createElement('div');
  overlay.id = 'notes-overlay';
  overlay.style.cssText = 'position:fixed;inset:0;z-index:9999;background:#1a1f2e;overflow:hidden;';

  const iframe = document.createElement('iframe');
  iframe.src = 'notes.html';
  iframe.style.cssText = 'position:absolute;inset:0;width:100%;height:100%;border:none;outline:none;display:block;';
  iframe.setAttribute('sandbox', 'allow-scripts allow-same-origin allow-forms allow-popups allow-top-navigation');

  const closeBtn = document.createElement('button');
  closeBtn.innerHTML = '&#x2715;';
  closeBtn.title = 'Zurück zum Tracker';
  closeBtn.style.cssText = 'position:absolute;top:1rem;right:1.25rem;z-index:10001;'
    + 'width:40px;height:40px;border-radius:50%;'
    + 'background:rgba(26,31,46,0.85);backdrop-filter:blur(8px);'
    + 'border:1.5px solid rgba(99,102,241,0.5);'
    + 'color:#818cf8;font-size:17px;font-weight:700;cursor:pointer;'
    + 'display:flex;align-items:center;justify-content:center;'
    + 'transition:background .18s,color .18s;';
  closeBtn.onmouseover = function() { this.style.background='#6366f1'; this.style.color='#fff'; };
  closeBtn.onmouseout  = function() { this.style.background='rgba(26,31,46,0.85)'; this.style.color='#818cf8'; };
    ...
    <div style="display:flex;align-items:center;gap:10px;margin-bottom:12px;
      ...
      <label style="display:flex;align-items:center;gap:7px;cursor:pointer;font-size:13px;color:var(--text2);user-select:none;">
        <input type="checkbox" ${allSelected ? 'checked' : ''}
          onchange="kaSelectAll(this.checked, [${visibleLeads.map(l=>`'${l.id}'`).join(',')}])"
          style="width:15px;height:15px;accent-color:#ef4444;cursor:pointer;">
        Alle markieren
      </label>
      <span style="font-size:12px;color:var(--text3);">${kaSelectedIds.size > 0 ? `${kaSelectedIds.size} ausgewählt` : ''}</span>
      <div style="flex:1;"></div>
      ${kaSelectedIds.size > 0 ? `
        <button onclick="kaDeleteSelected()"
          style="display:inline-flex;align-items:center;gap:6px;padding:7px 14px;border-radius:8px;
            border:1px solid rgba(239,68,68,0.4);background:rgba(239,68,68,0.08);
            color:#f87171;font-size:13px;font-weight:600;cursor:pointer;font-family:inherit;
            transition:all .15s;"
          onmouseover="this.style.background='rgba(239,68,68,0.18)'"
          onmouseout="this.style.background='rgba(239,68,68,0.08)'">
          <i class="ti ti-trash"></i> ${kaSelectedIds.size} löschen
        </button>
      ` : ''}
    </div>`;
}
