...
// CONTACTS PAGE
// ============================================================
window.renderContacts = function(filter = '', statusFilter = '', typeFilter = '') {
...
function renderCalendar() {
  ...
  // Alle Slots kombiniert: geschäftliche + private (bestätigte Freunde-Termine)
  // Anfragen (status:'anfrage') werden im Kalender nicht angezeigt, nur fixe Termine
  const allCalSlots = [
    ...callSlots,
    ...privateSlots.filter(s => s.type === 'fix' || s.status === 'confirmed'),
  ];
  ...
  // Ein Termin wird vom Lead Tracker in GCal eingetragen und speichert gcalEventIds.
  // Beim nächsten GCal-Refresh taucht er doppelt auf. Wir filtern ihn hier raus.
  ...
  const gcalByDay = days.map(d => googleCalendarEvents.filter(e => {
    if (!e.datetime || !e.datetime.includes('T')) return false;
    // Ausblenden wenn dieser GCal-Event bereits als Firestore-Slot dargestellt wird
  ...
  // ── GCal-Events ebenfalls in Zeitbereich einbeziehen (damit 19-Uhr-Termine sichtbar sind) ──
  days.forEach((d, i) => {
  ...
  document.getElementById('content').innerHTML = `
    ...
    <div style="display:flex;gap:14px;flex-wrap:wrap;margin-top:16px;padding:12px 16px;background:var(--surface);border:1px solid var(--border);border-radius:12px;align-items:center;">
      ...
      <span style="display:flex;align-items:center;gap:6px;font-size:13px;color:var(--text2);">
        <span style="width:12px;height:12px;border-radius:4px;background:#f59e0b;border:2px dashed #f59e0b;display:inline-block;box-sizing:border-box;"></span>🔒 Privat
      </span>
      <span style="display:flex;align-items:center;gap:6px;font-size:13px;color:var(--text2);">
        <span style="width:12px;height:12px;border-radius:4px;background:rgba(245,158,11,0.5);border:2px dashed var(--amber);display:inline-block;"></span>🔒 Privat
      </span>
...
window.confirmPrivateSlot = async function(slotId) {
  ...
  document.getElementById('pn_confirm_btn').onclick = async () => {
    ...
    // ── Kontakt anlegen / aktualisieren beim Bestätigen ──
    let confirmedContactId = slot.contactId || null;
    try {
      if (!confirmedContactId) {
        // Duplikat-Check via Telefonnummer
        if (slot.telefon) {
          try {
            const existing = await getDocs(query(collection(db, 'contacts'), where('telefon', '==', slot.telefon)));
            if (!existing.empty) confirmedContactId = existing.docs[0].id;
          } catch(_) {}
        }
        // Neu anlegen falls noch kein Kontakt existiert
        if (!confirmedContactId) {
          const ref = await addDoc(collection(db, 'contacts'), {
            ...
            email:          slot.email    || '',
            ort:            '',
            thema:          slot.apptType || slot.thema || '',
            ...
            history: [{
              ...
              note:     `[Bestätigter Termin] ${slot.apptType || ''}`,
              followup: null,
            }],
          });
          confirmedContactId = ref.id;
          console.log('Kontakt bei Bestätigung angelegt:', confirmedContactId);
        }
      }
    } catch(contactErr) {
      console.warn('Kontakt konnte nicht angelegt werden:', contactErr.message);
    }
...
// Zeigt ein Modal mit 2 freien Alternativslots zum Anbieten
window.offerPrivateAlternatives = function(slotId) {
  const slot = privateSlots.find(s => s.id === slotId);
  if (!slot) return;

  // Freie Slots finden (nächste 14 Tage, auf Basis SLOT_DURATION der Anfrage)
  const dur   = slot.apptDuration || 60;
  const now   = new Date();
  const limit = new Date(now.getTime() + 14 * 86400000);
  const freeSlots = [];

  // Alle belegten Zeiträume aufbauen (privat + geschäftlich)
  const allSlots = [...callSlots, ...privateSlots.filter(s => s.id !== slotId && (s.type === 'fix' || s.status === 'confirmed'))];
  const taken = allSlots.map(s => ({
    start: new Date(s.datetime),
    end:   new Date(new Date(s.datetime).getTime() + (s.apptDuration || 60) * 60000),
  }));

  // Kandidaten: stündlich von 08:00 – 22:00 in den nächsten 14 Tagen
  for (let d = 0; d < 14 && freeSlots.length < 6; d++) {
    const base = new Date(now);
    base.setDate(now.getDate() + d);
    base.setHours(8, 0, 0, 0);
    for (let h = 8; h + Math.ceil(dur / 60) <= 23 && freeSlots.length < 6; h++) {
      const cand = new Date(base); cand.setHours(h, 0, 0, 0);
      if (cand <= now) continue;
      if (cand >= limit) break;
      const candEnd = new Date(cand.getTime() + dur * 60000);
      const overlap = taken.some(t => cand < t.end && candEnd > t.start);
      if (!overlap) freeSlots.push(cand);
    }
  }

  showPrivateAlternativesModal(slot, freeSlots.slice(0, 6));
};
...
window.deletePrivateSlotFromCal = async function(slotId) {
  ...
  // ── Single-Point-of-Deletion: Firestore + alle verknüpften GCal-Kalender ──
  if (hasGcal) {
    toast('Wird aus Google Kalender gelöscht…');
    const deleted = await deleteGCalEvents(slot.gcalEventIds);
    if (deleted > 0) {
      // GCal-Events lokal entfernen, damit kein Re-Render-Duplikat bleibt
    ...
    <div style="background:var(--bg2);border:1px solid var(--border2);border-radius:18px;
      ...
      <div style="display:flex;flex-direction:column;gap:8px;margin-bottom:18px;">
        <label style="display:flex;align-items:flex-start;gap:10px;padding:10px 12px;background:var(--bg3);border:1px solid var(--border2);border-radius:8px;cursor:pointer;transition:background .15s;"
          ...
          <div>
            <div style="font-size:14px;font-weight:500;">📧 Terminbestätigung</div>
            <div style="font-size:11px;color:var(--text3);margin-top:2px;">Sofortige Bestätigung per ${slot.email ? 'E-Mail' : 'WhatsApp'}</div>
          </div>
        </label>
        <label style="display:flex;align-items:flex-start;gap:10px;padding:10px 12px;background:var(--bg3);border:1px solid var(--border2);border-radius:8px;cursor:pointer;transition:background .15s;"
          onmouseover="this.style.background='rgba(245,158,11,0.08)'" onmouseout="this.style.background='var(--bg3)'">
          ...
          <div>
            <div style="font-size:14px;font-weight:500;">🔔 Terminerinnerung</div>
            <div style="font-size:11px;color:var(--text3);margin-top:2px;">Erinnerungsnachricht am Termintag um 09:00 Uhr</div>
          </div>
        </label>
        <label style="display:flex;align-items:flex-start;gap:10px;padding:10px 12px;background:var(--bg3);border:1px solid var(--border2);border-radius:8px;cursor:pointer;transition:background .15s;"
          onmouseover="this.style.background='rgba(139,92,246,0.08)'" onmouseout="this.style.background='var(--bg3)'">
          ...
          <div>
            <div style="font-size:14px;font-weight:500;">📲 Terminbenachrichtigung</div>
            <div style="font-size:11px;color:var(--text3);margin-top:2px;">Separate Info-Nachricht über alle Termindetails</div>
          </div>
        </label>
      </div>
      <div style="display:flex;gap:8px;">
...
// Alternativtermine für Mandanten anbieten
window.offerBusinessAlternatives = function(slotId) {
  // Strikte Trennung: NUR in callSlots suchen
  const slot = callSlots.find(s => s.id === slotId);
  if (!slot) { toast('Anfrage nicht gefunden.', 'error'); return; }
  ...
  // Alle belegten Zeiträume (NUR geschäftliche Slots für Mandanten-Kalender)
  const takenSlots = callSlots.filter(s => s.id !== slotId && (s.type === 'fix' || s.status === 'confirmed'));
  const taken = takenSlots.map(s => ({
    start: new Date(s.datetime),
    end:   new Date(new Date(s.datetime).getTime() + (s.apptDuration || 60) * 60000),
  }));

  // Kandidaten: stündlich von 08:00 – 18:00 (Geschäftszeiten)
  for (let d = 0; d < 14 && freeSlots.length < 6; d++) {
    const base = new Date(now);
    base.setDate(now.getDate() + d);
    base.setHours(8, 0, 0, 0);
    for (let h = 8; h + Math.ceil(dur / 60) <= 19 && freeSlots.length < 6; h++) {
      const cand = new Date(base); cand.setHours(h, 0, 0, 0);
      if (cand <= now) continue;
      if (cand >= limit) break;
  ...
  showBusinessAlternativesModal(slot, freeSlots.slice(0, 6));
};
