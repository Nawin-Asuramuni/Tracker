...
window.confirmPrivateSlot = async function(slotId) {
  ...
  if (!confirm(`Termin für ${slot.vorname} bestätigen?

${datum}
${von} – ${bis} Uhr

Danach wird WhatsApp geöffnet.`)) return;
...
window.confirmBusinessSlot = async function(slotId) {
  ...
  if (!confirm(`Termin für ${slot.vorname} ${slot.nachname || ''} bestätigen?\n\n${datum}\n${von} – ${bis} Uhr\n\nDanach wird WhatsApp/E-Mail geöffnet.`)) return;
  ...
  try {
    ...
    if (slot.contactId) {
      await updateDoc(doc(db, 'contacts', slot.contactId), {
        status: 'termin',
        // Datentrennung: Mandanten bleiben als type:'business'
        type: 'business',
      }).catch(() => {});
    }
    toast(`✅ Termin für ${slot.vorname} bestätigt!`, 'success');
  ...
  // Bestätigung: E-Mail wenn vorhanden, sonst WhatsApp
  const settings = JSON.parse(localStorage.getItem('crmSettings') || '{}');
