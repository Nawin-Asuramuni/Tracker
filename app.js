...
// ============================================================
let calWeekOffset = 0;
window.calWeekOffset = 0; // exposed for inline onclick handlers

window.setCalWeekOffset = function(w) {
  calWeekOffset = w;
  window.calWeekOffset = w;
  renderCalendar();
};
...
function renderCalendar() {
  ...
  // Count slots per day for summary
  const slotsByDay = days.map(d => callSlots.filter(s => new Date(s.datetime).toDateString() === d.toDateString()));
  const totalWeek = slotsByDay.reduce((a, b) => a + b.length, 0);

  document.getElementById('content').innerHTML = `
    <!-- Header -->
    <div style="display:flex;align-items:center;gap:10px;margin-bottom:6px;flex-wrap:wrap;">
      <button class="btn-icon" id="prevWeek" title="Vorherige Woche"><i class="ti ti-chevron-left"></i></button>
      <div style="flex:1;min-width:0;">
        <div style="font-size:17px;font-weight:600;">${weekStr}</div>
        <div style="font-size:12px;color:var(--text3);margin-top:1px;">${weekLabel} · ${totalWeek} Termin${totalWeek!==1?'e':''}</div>
      </div>
      <button class="btn-icon" id="nextWeek" title="Nächste Woche"><i class="ti ti-chevron-right"></i></button>
      ${calWeekOffset !== 0 ? `<button class="btn-ghost" style="font-size:12px;padding:5px 10px;" id="goToday"><i class="ti ti-calendar-event"></i> Heute</button>` : ''}
    ...
    <div style="display:flex;gap:6px;margin-bottom:16px;flex-wrap:wrap;">
      ${[-1,0,1,2].map(w => {
        ...
        const lbl = w === -1 ? 'Letzte' : w === 0 ? 'Diese Woche' : w === 1 ? 'Nächste' : 'Übernächste';
        return `<button onclick="setCalWeekOffset(${w})"
          style="font-size:12px;padding:5px 12px;border-radius:20px;border:1px solid ${active?'var(--accent)':'var(--border2)'};
          background:${active?'rgba(59,130,246,0.15)':'transparent'};color:${active?'var(--accent)':'var(--text2)'};cursor:pointer;transition:all .15s;">${lbl}</button>`;
      }).join('')}
    </div>
    ...
    <div class="week-wrap">
      <div style="display:grid;grid-template-columns:repeat(7,1fr);gap:6px;">
        ${days.map((d, i) => {
          ...
          const timeLabel = dc.start && dc.end ? `${dc.start}–${dc.end}` : '';
          const breakLabel = dc.breakStart && dc.breakEnd ? `🍽 ${dc.breakStart}–${dc.breakEnd}` : '';
          const daySlots = slotsByDay[i];

          return `
          <div style="
            background:${isUnavailable||isBlocked ? 'var(--bg)' : isToday ? 'rgba(59,130,246,0.06)' : 'var(--surface)'};
            border:1px solid ${isToday ? 'rgba(59,130,246,0.4)' : 'var(--border)'};
            border-radius:12px;min-height:220px;padding-bottom:6px;
            ${isUnavailable||isBlocked ? 'opacity:0.5;' : ''}
            display:flex;flex-direction:column;overflow:hidden;
          ">
            <!-- Day header -->
            <div style="padding:8px 10px 6px;border-bottom:1px solid var(--border);${isToday?'background:rgba(59,130,246,0.08);':''}">
              <div style="font-size:10px;text-transform:uppercase;letter-spacing:.06em;color:${isToday?'var(--accent)':'var(--text3)'};">${names[i]}</div>
              <div style="font-size:${isToday?'20':'17'}px;font-weight:700;color:${isToday?'var(--accent)':'var(--text)'};line-height:1.2;">${d.getDate()}</div>
              ${isBlocked
                ? `<div style="font-size:9px;color:var(--red);margin-top:2px;">🚫 Blockiert</div>`
                : isUnavailable
                  ? `<div style="font-size:9px;color:var(--text3);margin-top:2px;">Nicht verfügbar</div>`
                  : timeLabel
                    ? `<div style="font-size:9px;color:var(--text3);margin-top:2px;">${timeLabel}${breakLabel ? ' · ' + breakLabel : ''}</div>`
                    : ''
              }
            </div>

            <!-- Slots -->
            <div style="padding:6px;display:flex;flex-direction:column;gap:4px;flex:1;">
              ${daySlots.map(sl => {
                const c = contacts.find(x => x.id === sl.contactId);
                const name = c ? `${c.vorname} ${c.nachname}` : '?';
                const time = new Date(sl.datetime).toLocaleTimeString('de-DE', { hour:'2-digit', minute:'2-digit' });
                const color = sl.apptColor || (sl.type === 'flex' ? '#f59e0b' : '#3b82f6');
                const bgAlpha = '26'; // ~15% opacity hex
                return `
                <div onclick="showContact('${sl.contactId||''}')" style="
                  padding:6px 8px;border-radius:7px;cursor:pointer;transition:opacity .15s;
                  background:${color}${bgAlpha};
                  border-left:3px solid ${color};
                " title="${sl.note || name}">
                  <div style="font-size:11px;font-weight:600;color:${color};white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${time}${sl.apptType ? ' · ' + sl.apptType : ''}</div>
                  <div style="font-size:11px;color:var(--text);margin-top:1px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${name}</div>
                  ${sl.apptDuration ? `<div style="font-size:10px;color:var(--text3);margin-top:1px;">⏱ ${sl.apptDuration} Min.</div>` : ''}
                </div>`;
              }).join('')}

              ${!isUnavailable && !isBlocked ? `
              <button onclick="openCallSlotModalForDate('${d.toISOString()}')"
                style="width:100%;background:none;border:1px dashed var(--border2);color:var(--text3);border-radius:7px;
                padding:5px;font-size:11px;cursor:pointer;margin-top:2px;transition:all .15s;"
    ...
    <!-- Legend -->
    <div style="display:flex;gap:12px;flex-wrap:wrap;margin-top:14px;padding:10px 14px;background:var(--surface);border:1px solid var(--border);border-radius:10px;align-items:center;">
      <span style="font-size:12px;color:var(--text3);font-weight:500;">Legende:</span>
      ${(s.apptTypes||[]).map(t => `<span style="display:flex;align-items:center;gap:5px;font-size:12px;color:var(--text2);">
        <span style="width:10px;height:10px;border-radius:3px;background:${t.color};display:inline-block;"></span>${t.name}
      </span>`).join('')}
      <span style="display:flex;align-items:center;gap:5px;font-size:12px;color:var(--text2);">
        <span style="width:10px;height:10px;border-radius:3px;background:var(--accent);display:inline-block;"></span>Fix
      </span>
      <span style="display:flex;align-items:center;gap:5px;font-size:12px;color:var(--text2);">
        <span style="width:10px;height:10px;border-radius:3px;background:var(--amber);display:inline-block;"></span>Flexibel
      </span>
      <span style="margin-left:auto;font-size:12px;color:var(--text3);">
        <button onclick="navigate('settings')" style="background:none;border:none;color:var(--accent);cursor:pointer;font-size:12px;padding:0;"><i class="ti ti-settings" style="font-size:12px;"></i> Verfügbarkeit & Terminarten</button>
      </span>
    </div>
  `;

  document.getElementById('prevWeek').addEventListener('click', () => { calWeekOffset--; window.calWeekOffset = calWeekOffset; renderCalendar(); });
  document.getElementById('nextWeek').addEventListener('click', () => { calWeekOffset++; window.calWeekOffset = calWeekOffset; renderCalendar(); });
  document.getElementById('goToday')?.addEventListener('click', () => { calWeekOffset = 0; window.calWeekOffset = 0; renderCalendar(); });
}
