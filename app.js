...
window.openContactModal = function(id = null) {
  ...
  if (catSel) {
    ...
    if (opts[1]) opts[1].textContent = '👤 ' + (s.catLabelMandantPrivat || 'Mandant privat');
    if (opts[2]) opts[2].textContent = '👥 ' + (s.catLabelMitarbeiter || 'Mitarbeiter');
    if (opts[3]) opts[3].textContent = '🤝 ' + (s.catLabelMandantMitarbeiter || 'Mandant von Mitarbeiter');
  }
...
function openAltPickerModal({ slot, isPrivate, takenSlots, isStaff = false }) {
  ...
  function sendAlt(via) {
    ...
    const labels = chosen.map(c => '📅 ' + fmtChosen(c));
    let msg;
    if (isStaff) {
      msg = `Hey ${slot.vorname} 👋\n\nDer angeforderte Termin (${slot.apptType || 'Termin'}) klappt leider nicht. Ich habe für uns folgende freie Zeitfenster:\n\n${labels.join('\n')}\n\nSag mir kurz, was dir passt! 🙌`;
    } else if (isPrivate) {
...
function getCategoryLabels() {
  const s = JSON.parse(localStorage.getItem('crmSettings') || '{}');
  return {
    mandantPrivat:     s.catLabelMandantPrivat     || 'Mandant privat',
 ...
 * category: 'mandantPrivat' | 'mitarbeiter' | 'mandantMitarbeiter' | null
 */
...
function categoryBadge(category) {
  ...
  const map = {
    mandantPrivat:      { label: labels.mandantPrivat,      color: '#f59e0b', icon: 'ti-user-check' },
...
window.renderContacts = function(filter = '', statusFilter = '', typeFilter = '', catFilter = '') {
  ...
  const businessCount   = contacts.filter(c => c.type !== 'private' && !c.privateContact).length;
  const catCounts = {
    mandantPrivat:      contacts.filter(c => c.contactCategory === 'mandantPrivat').length,
  ...
  document.getElementById('content').innerHTML = `
    ...
    <div style="display:flex;gap:6px;margin-bottom:14px;flex-wrap:wrap;align-items:center;">
      ...
      ${Object.entries({
        mandantPrivat: labels.mandantPrivat,
            ...
            <tr style="${isPrivate ? 'background:rgba(245,158,11,0.04);' : ''}">
              ...
              <td>
                ...
                <select onchange="setContactCategory('${c.id}',this.value)"
                  ...
                  <option value="mandantPrivat"      ${c.contactCategory==='mandantPrivat'?'selected':''}>${labels.mandantPrivat}</option>
...
function buildCategorySettingsHTML(s) {
  ...
  return `
    ...
    <div class="settings-section" id="categorySettingsSection">
      ...
      <p style="font-size:13px;color:var(--text2);margin-bottom:16px;">
        Benenne die drei Kontaktkategorien nach deinen Bedürfnissen um.
      ...
      <div class="form-grid">
        <div class="field">
          ...
          <input type="text" id="s_catLabelMandantPrivat"
            value="${s.catLabelMandantPrivat || 'Mandant privat'}"
            placeholder="z.B. Mandant privat" />
        </div>
        <div class="field">
          <label style="display:flex;align-items:center;gap:6px;">
      ...
      <p style="font-size:12px;color:var(--text3);margin-top:8px;">
        ...
        Tipp: Kategorie 3 eignet sich ideal für Mandanten, die über einen Empfehlungsmanager vermittelt wurden.
      </p>
    </div>
  `;
}
...
function _extRenderSettings() {
  ...
  requestAnimationFrame(() => {
    ...
    // Sektion vor dem Speichern-Button einfügen
    ...
    // Speichern-Button: Kategorie-Labels mitspeichern
    const originalSaveClick = saveBtn?._catPatched;
    if (saveBtn && !originalSaveClick) {
      saveBtn._catPatched = true;
      saveBtn.addEventListener('click', () => {
        const existing = JSON.parse(localStorage.getItem('crmSettings') || '{}');
        existing.catLabelMandantPrivat      = document.getElementById('s_catLabelMandantPrivat')?.value.trim()      || 'Mandant privat';
        ...
        localStorage.setItem('crmSettings', JSON.stringify(existing));
        // toast wird bereits vom Original ausgelöst
      }, { capture: false }); // nach dem Original-Handler ausführen
    }
  });
}
