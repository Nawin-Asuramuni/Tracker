...
// SIDEBAR
// ============================================================
function _forceSidebar() {
  const sidebar = document.getElementById('sidebar');
  const overlay = document.getElementById('overlay');
  // Sidebar muss über den Panels liegen (z-index 999/998)
  sidebar.style.zIndex = '1001';
  overlay.style.zIndex = '1000';
  sidebar.classList.add('open');
  overlay.classList.add('show');
}
window._forceSidebar = _forceSidebar;
function openSidebar() {
  // Sidebar nur öffnen wenn eingeloggt
  if (!auth.currentUser || auth.currentUser.email !== ALLOWED_EMAIL) return;
  // Standard z-index zurücksetzen
...
// Hamburger-Menü im Social Media Panel
const smMenuBtn = document.getElementById('smPanelMenuBtn');
if (smMenuBtn) smMenuBtn.addEventListener('click', () => {
  if (!auth.currentUser || auth.currentUser.email !== ALLOWED_EMAIL) return;
  _forceSidebar();
});

// Hamburger-Menü im Notizen Panel
const notesMenuBtn = document.getElementById('notesPanelMenuBtn');
if (notesMenuBtn) notesMenuBtn.addEventListener('click', () => {
  if (!auth.currentUser || auth.currentUser.email !== ALLOWED_EMAIL) return;
  _forceSidebar();
});
