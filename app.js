...
document.querySelectorAll('.nav-item').forEach(n => {
  n.addEventListener('click', e => {
    // Externe Links (z.B. notes.html) nicht abfangen
    if (n.classList.contains('nav-external')) return;
    e.preventDefault();
...
{signatur}`;
      ...
      <div style="display:flex;flex-direction:column;gap:12px;">
        ...
        <div style="background:var(--bg3);border:1px solid var(--border);border-radius:10px;padding:14px 16px;">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:4px;flex-wrap:wrap;gap:8px;">
            <div style="font-weight:600;font-size:14px;">🎞️ Präsentationen</div>
            <button type="button" id="addPresBtn"
              ...
              color:var(--purple,#8b5cf6);font-size:12px;font-weight:600;cursor:pointer;font-family:inherit;transition:all .15s;">
              <i class="ti ti-plus"></i> Präsentation hinzufügen
            </button>
          </div>
          <p style="font-size:12px;color:var(--text2);margin-bottom:14px;">
            HTML-Dateien hochladen – erscheinen als Karten im Präsentations-Bereich der Website. Titel &amp; Beschreibung sind editierbar.
          </p>
          <div id="pres-upload-list" style="display:flex;flex-direction:column;gap:10px;"></div>
          <div id="pres-drop-zone" style="
            border:2px dashed rgba(139,92,246,0.25);border-radius:10px;padding:28px 16px;
            text-align:center;cursor:pointer;transition:all .2s;margin-top:10px;position:relative;">
            <input type="file" id="pres-drop-input" accept=".html"
              style="position:absolute;inset:0;opacity:0;cursor:pointer;z-index:1;" multiple />
            <i class="ti ti-file-code" style="font-size:28px;color:var(--purple,#8b5cf6);display:block;margin-bottom:8px;"></i>
            <div style="font-size:13px;font-weight:600;color:var(--text);">HTML-Dateien hier ablegen oder klicken</div>
            <div style="font-size:11px;color:var(--text3);margin-top:4px;">Mehrere Dateien gleichzeitig möglich</div>
          </div>
        </div>
  ...
  document.getElementById('saveSettings').addEventListener('click', () => {
    ...
    const settings = {
      myEmail:            document.getElementById('s_myEmail')?.value.trim() || s.myEmail || '',
      emailSig:           document.getElementById('s_emailSig')?.value || s.emailSig || '',
      autoConfirm:        document.getElementById('s_autoConfirm')?.checked || false,
      autoReminder:       document.getElementById('s_autoReminder')?.checked || false,
      confirmSubject:     document.getElementById('s_confirmSubject')?.value.trim() || s.confirmSubject || 'Terminbestätigung – {datum}',
      emailConfirmText:   document.getElementById('s_emailConfirmText')?.value || s.emailConfirmText || '',
      notifySubject:      document.getElementById('s_notifySubject')?.value.trim() || s.notifySubject || 'Ihr Termin am {datum}',
      emailNotifyText:    document.getElementById('s_emailNotifyText')?.value || s.emailNotifyText || '',
      reminderSubject:    document.getElementById('s_reminderSubject')?.value.trim() || s.reminderSubject || '',
      emailReminderText:  document.getElementById('s_emailReminderText')?.value || s.emailReminderText || '',
      dayConfigs,
      availDays,
      availStart,
      availEnd,
      blockedDates,
      gcalOAuthClientId: document.getElementById('s_gcalOAuthClientId')?.value.trim() || s.gcalOAuthClientId || '',
      gcalKey:      document.getElementById('s_gcalKey')?.value.trim() || s.gcalKey || '',
      gcalId:       document.getElementById('s_gcalId')?.value.trim() || s.gcalId || '',
      gcalKey2:     document.getElementById('s_gcalKey2')?.value.trim() || s.gcalKey2 || '',
      gcalId2:      document.getElementById('s_gcalId2')?.value.trim() || s.gcalId2 || 'nawin.dep@gmail.com',
      gmailSender:  document.getElementById('s_gmailSender')?.value.trim() || s.gmailSender || 'nawin.telis@gmail.com',
      msClientId:   document.getElementById('s_msClientId')?.value.trim() || s.msClientId || '',
      msEmail:      document.getElementById('s_msEmail')?.value.trim() || s.msEmail || '',
      mondayKey:    document.getElementById('s_mondayKey')?.value.trim() || s.mondayKey || '',
      mondayBoardId: document.getElementById('s_mondayBoard')?.value.trim() || s.mondayBoardId || '',
      apptTypes,
      staffApptTypes,
      verkehrsmittel: document.getElementById('s_verkehrsmittel')?.value || s.verkehrsmittel || 'auto',
      pufferMinuten:  parseInt(document.getElementById('s_pufferMinuten')?.value) || s.pufferMinuten || 15,
      gmapsKey:            document.getElementById('s_gmapsKey')?.value.trim() || s.gmapsKey || '',
      publicWeekPassword:  document.getElementById('s_publicWeekPassword')?.value || s.publicWeekPassword || '',
    };
    ...
    // Präsentationen – aus dem live State übernehmen
    siteSettings.presentations = window._presState || prevSite.presentations || [];

    try {
      localStorage.setItem('siteAdminSettings', JSON.stringify(siteSettings));
    } catch(quotaErr) {
      // localStorage voll (typisch bei großen HTML-Präsentationen)
      // Präsentationen ohne htmlData speichern und Nutzer informieren
      const slim = Object.assign({}, siteSettings, {
        presentations: (siteSettings.presentations || []).map(p => Object.assign({}, p, { htmlData: p.htmlData ? '[zu groß – extern verlinken]' : '' }))
      });
      try { localStorage.setItem('siteAdminSettings', JSON.stringify(slim)); } catch(e2) {}
      toast('⚠️ Präsentations-HTML zu groß für localStorage (>5 MB). Andere Einstellungen wurden gespeichert.', 'error');
      syncPublicSettings();
    ...
    toast('Einstellungen gespeichert!');
      ...
      console.error('saveSettings Fehler:', err);
  ...
  (function renderLegalZones() {
    ...
    container.innerHTML = ['impressum','datenschutz'].map(key => {
      ...
      return '<div style="margin-bottom:12px;">'
        ...
        + 'style="border:2px dashed rgba(99,102,241,0.2);border-radius:8px;padding:14px 16px;'
  ...
  function _legalHandleFile(key, file) {
    ...
    reader.onload = e => {
      ...
      // Sofort in localStorage persistieren
      try {
        const site = JSON.parse(localStorage.getItem('siteAdminSettings') || '{}');
        site[key] = e.target.result;
        site[key + 'Name'] = file.name;
        localStorage.setItem('siteAdminSettings', JSON.stringify(site));
        toast(`"${file.name}" gespeichert.`);
      } catch(quotaE) {
        toast('⚠️ Datei zu groß für localStorage – bitte als Link auf GitHub ablegen.', 'error');
      }
    };
    reader.readAsDataURL(file);
  }
  ...
  function _legalRemove(key) {
    ...
    // Sofort in localStorage loeschen – kein Speichern-Klick noetig
    try {
      const site = JSON.parse(localStorage.getItem('siteAdminSettings') || '{}');
      site[key] = '';
      site[key + 'Name'] = '';
  ...
  // ── Präsentationen Upload-Manager ─────────────────────────
  (function initPresManager() {
    const ws3 = JSON.parse(localStorage.getItem('siteAdminSettings') || '{}');
    // Live state: array of {id, title, description, htmlData, fileName}
    window._presState = (ws3.presentations || []).map(p => Object.assign({}, p));

    function _renderPresList() {
      const list = document.getElementById('pres-upload-list');
      if (!list) return;
      if (window._presState.length === 0) {
        list.innerHTML = '<div style="font-size:12px;color:var(--text3);text-align:center;padding:6px 0;">Noch keine Präsentationen hochgeladen.</div>';
        return;
      }
      list.innerHTML = window._presState.map((p, i) => `
        <div data-pres-idx="${i}" style="
          background:var(--bg2);border:1px solid var(--border2);border-radius:10px;
          padding:12px 14px;display:flex;flex-direction:column;gap:8px;">
          <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
            <i class="ti ti-file-code" style="color:var(--purple,#8b5cf6);font-size:18px;flex-shrink:0;"></i>
            <span style="font-size:12px;font-weight:600;color:var(--text);flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="${p.fileName||''}">${p.fileName || 'Unbekannt'}</span>
            <button type="button" class="pres-del-btn" data-idx="${i}"
              style="background:none;border:none;color:var(--red,#ef4444);cursor:pointer;font-size:18px;line-height:1;padding:2px 4px;flex-shrink:0;" title="Entfernen">
              <i class="ti ti-trash"></i>
            </button>
          </div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
            <div>
              <label style="font-size:11px;color:var(--text3);font-weight:600;display:block;margin-bottom:3px;">TITEL</label>
              <input type="text" class="pres-title-inp" data-idx="${i}"
                value="${(p.title||'').replace(/"/g,'&quot;')}"
                placeholder="z.B. FPG Präsentation"
                style="width:100%;background:var(--bg3);border:1px solid var(--border2);color:var(--text);border-radius:7px;padding:6px 9px;font-size:13px;font-family:inherit;outline:none;" />
            </div>
            <div>
              <label style="font-size:11px;color:var(--text3);font-weight:600;display:block;margin-bottom:3px;">BESCHREIBUNG</label>
              <input type="text" class="pres-desc-inp" data-idx="${i}"
                value="${(p.description||'').replace(/"/g,'&quot;')}"
                placeholder="Kurze Beschreibung..."
                style="width:100%;background:var(--bg3);border:1px solid var(--border2);color:var(--text);border-radius:7px;padding:6px 9px;font-size:13px;font-family:inherit;outline:none;" />
            </div>
          </div>
          ${p.htmlData ? `<div style="font-size:11px;color:var(--green,#22c55e);"><i class="ti ti-check-circle"></i> HTML geladen (${Math.round(p.htmlData.length/1024)} KB)</div>` : `<div style="font-size:11px;color:var(--amber,#f59e0b);"><i class="ti ti-alert-circle"></i> Noch keine Datei – erneut hochladen</div>`}
        </div>`).join('');

      // Live-sync title / description changes back to state
      list.querySelectorAll('.pres-title-inp').forEach(inp => {
        inp.addEventListener('input', () => {
          window._presState[inp.dataset.idx].title = inp.value;
        });
      });
      list.querySelectorAll('.pres-desc-inp').forEach(inp => {
        inp.addEventListener('input', () => {
          window._presState[inp.dataset.idx].description = inp.value;
        });
      });
      // Delete buttons – sofort in localStorage persistieren
      list.querySelectorAll('.pres-del-btn').forEach(btn => {
        btn.addEventListener('click', () => {
          const idx = parseInt(btn.dataset.idx);
          const name = window._presState[idx]?.fileName || 'Präsentation';
          window._presState.splice(idx, 1);
          _renderPresList();
          try {
            const site = JSON.parse(localStorage.getItem('siteAdminSettings') || '{}');
            site.presentations = window._presState;
            localStorage.setItem('siteAdminSettings', JSON.stringify(site));
            toast(`"${name}" gelöscht.`);
          } catch(e) {
            toast('Löschen fehlgeschlagen: ' + e.message, 'error');
    ...
    function _addPresFiles(files) {
      Array.from(files).forEach(file => {
        if (!file.name.endsWith('.html')) return;
        const reader = new FileReader();
        reader.onload = e => {
          const existingIdx = window._presState.findIndex(p => p.fileName === file.name);
          const entry = {
          ...
          if (existingIdx >= 0) {
            window._presState[existingIdx] = entry;
          } else {
            window._presState.push(entry);
          }
          _renderPresList();
          // Sofort in localStorage persistieren
          try {
            ...
            site.presentations = window._presState;
            localStorage.setItem('siteAdminSettings', JSON.stringify(site));
            toast(`"${file.name}" ${existingIdx >= 0 ? 'aktualisiert' : 'hinzugefügt'} & gespeichert.`);
          } catch(quotaE) {
            toast('⚠️ HTML zu groß für localStorage – Präsentation wurde nicht gespeichert.', 'error');
          }
        };
    ...
    // Drop zone
    const dropZone = document.getElementById('pres-drop-zone');
    const dropInput = document.getElementById('pres-drop-input');
    if (dropZone && dropInput) {
      dropZone.addEventListener('dragover', e => {
        e.preventDefault();
        dropZone.style.borderColor = 'var(--purple,#8b5cf6)';
        dropZone.style.background  = 'rgba(139,92,246,0.07)';
      });
      ...
      dropZone.addEventListener('drop', e => {
        ...
        dropZone.style.borderColor = '';
        dropZone.style.background  = '';
        _addPresFiles(e.dataTransfer.files);
      });
      dropInput.addEventListener('change', e => {
        _addPresFiles(e.target.files);
        e.target.value = '';
      });
    }

    // "Präsentation hinzufügen" button triggers the same file input
