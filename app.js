...
async function syncToGoogleCalendar(slotData, contactData) {
  ...
  // Sync in beide Kalender (Kalender 1 + Kalender 2)
  const calendarIds = [s.gcalId, s.gcalId2].filter(Boolean);
  if (calendarIds.length === 0) return;

  let anySuccess = false;
  for (const calId of calendarIds) {
    const url = `https://www.googleapis.com/calendar/v3/calendars/${encodeURIComponent(calId)}/events?key=${s.gcalKey}`;
    try {
...
async function loadGoogleCalendarEvents() {
  ...
  for (const calId of calendarIds) {
    const url = `https://www.googleapis.com/calendar/v3/calendars/${encodeURIComponent(calId)}/events?key=${s.gcalKey}&timeMin=${timeMin}&timeMax=${timeMax}&singleEvents=true&orderBy=startTime&maxResults=100`;
    try {
      const res = await fetch(url);
      if (res.ok) {
        ...
        const events = (data.items || []).map(e => ({
          ...
          calendarId: calId,
          calendarLabel: calId === s.gcalId ? 'Kalender 1' : 'Kalender 2',
          source: 'google',
          htmlLink: e.htmlLink,
        }));
  ...
  document.getElementById('content').innerHTML = `
    ...
    <div class="settings-section">
      ...
      <div class="form-grid">
        ...
        <div class="field full"><label>Kalender 1 – Calendar-ID</label>
          <input type="text" id="s_gcalId" placeholder="z.B. nawin.telis@gmail.com" value="${s.gcalId||'nawin.telis@gmail.com'}" />
        </div>
        <div class="field full"><label>Kalender 2 – Calendar-ID</label>
          <input type="text" id="s_gcalId2" placeholder="z.B. nawin.dep@gmail.com" value="${s.gcalId2||'nawin.dep@gmail.com'}" />
        </div>
      </div>
      ...
      <button class="btn-ghost" id="loadGcalBtn" style="margin-top:10px;font-size:13px;" ${!s.gcalKey?'disabled style="opacity:0.5;"`:''}>
