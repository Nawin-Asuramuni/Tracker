...
// CONTACTS PAGE
// ============================================================
function renderContacts(filter = '', statusFilter = '', typeFilter = '') {
  const filtered = contacts.filter(c => {
    ...
    const matchType = !typeFilter || (typeFilter === 'private' ? c.type === 'private' || c.privateContact === true : c.type !== 'private' && !c.privateContact);
    return matchSearch && matchStatus && matchType;
  });

  const privateCount = contacts.filter(c => c.type === 'private' || c.privateContact === true).length;
  const businessCount = contacts.filter(c => c.type !== 'private' && !c.privateContact).length;
  ...
  document.getElementById('content').innerHTML = `
    ...
    <div style="display:flex;gap:8px;margin-bottom:14px;flex-wrap:wrap;">
      <button onclick="renderContacts(document.getElementById('searchInput')?.value||'', document.getElementById('statusFilter')?.value||'', '')"
        style="font-size:13px;padding:6px 14px;border-radius:20px;border:1.5px solid ${!typeFilter?'var(--accent)':'var(--border2)'};
        background:${!typeFilter?'rgba(59,130,246,0.12)':'transparent'};color:${!typeFilter?'var(--accent)':'var(--text2)'};
        cursor:pointer;font-weight:${!typeFilter?'600':'400'};transition:all .15s;">
        Alle <span style="font-size:11px;opacity:.7;">(${contacts.length})</span>
      </button>
      <button onclick="renderContacts(document.getElementById('searchInput')?.value||'', document.getElementById('statusFilter')?.value||'', 'business')"
        style="font-size:13px;padding:6px 14px;border-radius:20px;border:1.5px solid ${typeFilter==='business'?'var(--purple)':'var(--border2)'};
        background:${typeFilter==='business'?'rgba(139,92,246,0.12)':'transparent'};color:${typeFilter==='business'?'var(--purple)':'var(--text2)'};
        cursor:pointer;font-weight:${typeFilter==='business'?'600':'400'};transition:all .15s;">
        <i class="ti ti-briefcase" style="font-size:12px;"></i> Mandanten <span style="font-size:11px;opacity:.7;">(${businessCount})</span>
      </button>
      <button onclick="renderContacts(document.getElementById('searchInput')?.value||'', document.getElementById('statusFilter')?.value||'', 'private')"
        style="font-size:13px;padding:6px 14px;border-radius:20px;border:1.5px solid ${typeFilter==='private'?'var(--amber)':'var(--border2)'};
        background:${typeFilter==='private'?'rgba(245,158,11,0.12)':'transparent'};color:${typeFilter==='private'?'var(--amber)':'var(--text2)'};
        cursor:pointer;font-weight:${typeFilter==='private'?'600':'400'};transition:all .15s;">
        🔒 Privat <span style="font-size:11px;opacity:.7;">(${privateCount})</span>
      </button>
    </div>
    ...
    <div class="contacts-table-wrap">
      <table class="contacts-table">
        ...
        <tbody>
          ${filtered.map(c => {
            const isPrivate = c.type === 'private' || c.privateContact === true;
            return `
            <tr style="${isPrivate ? 'background:rgba(245,158,11,0.04);' : ''}">
              <td>
                <div class="contact-name" style="display:flex;align-items:center;gap:6px;">
                  ${isPrivate ? '<span title="Privater Kontakt" style="font-size:11px;background:rgba(245,158,11,0.15);color:var(--amber);border:1px solid rgba(245,158,11,0.3);border-radius:4px;padding:1px 5px;font-weight:600;flex-shrink:0;">🔒 Privat</span>' : ''}
  ...
  document.getElementById('searchInput').addEventListener('input', e => renderContacts(e.target.value, document.getElementById('statusFilter').value, typeFilter));
  document.getElementById('statusFilter').addEventListener('change', e => renderContacts(document.getElementById('searchInput').value, e.target.value, typeFilter));
}
...
function renderCalendar() {
  ...
  // Private Termine für den Wochenplan (bestätigte + anfragen, strikt getrennt)
...
{signatur}`;
  ...
  document.getElementById('content').innerHTML = `
    ...
    <div class="settings-section">
      ...
      <div style="display:flex;flex-direction:column;gap:12px;">
        ...
        <div style="background:var(--bg3);border:1px solid var(--border);border-radius:10px;padding:14px 16px;">
          <div style="font-weight:600;font-size:14px;margin-bottom:4px;">📄 Impressum & Datenschutz</div>
          <p style="font-size:12px;color:var(--text2);margin-bottom:14px;">PDF-Dateien hochladen – werden auf der öffentlichen Website verlinkt.</p>
          <div id="legal-upload-zones"></div>
        </div>
  ...
  document.getElementById('saveSettings').addEventListener('click', () => {
    ...
    for (const key of ['impressum', 'datenschutz']) {
      const zone = document.querySelector(`.legal-drop-zone[data-key="${key}"]`);
      if (!zone) { siteSettings[key] = prevSite[key] || ''; siteSettings[key+'Name'] = prevSite[key+'Name'] || ''; continue; }
      if (zone._pendingData === '__remove__') {
        siteSettings[key] = '';
        siteSettings[key+'Name'] = '';
      } else if (zone._pendingData) {
        siteSettings[key] = zone._pendingData;
        siteSettings[key+'Name'] = zone._pendingName || key + '.pdf';
      } else {
        siteSettings[key] = prevSite[key] || '';
        siteSettings[key+'Name'] = prevSite[key+'Name'] || '';
      }
    }
  ...
  // ── Impressum & Datenschutz Upload-Zonen rendern ──────────────
  (function renderLegalZones() {
    const container = document.getElementById('legal-upload-zones');
    if (!container) return;
    const ws2 = JSON.parse(localStorage.getItem('siteAdminSettings') || '{}');
    container.innerHTML = ['impressum','datenschutz'].map(key => {
      const label    = key === 'impressum' ? 'Impressum' : 'Datenschutzerklärung';
      const existing = ws2[key + 'Name'] || '';
      const fileInfo = existing
        ? '<div style="display:flex;align-items:center;justify-content:center;gap:8px;font-size:13px;">'
          + '<i class="ti ti-file-filled" style="color:#3b82f6;font-size:18px;"></i>'
          + '<span class="legal-filename-' + key + '" style="color:var(--text);">' + existing + '</span>'
          + '<button type="button" class="legal-remove-btn" data-key="' + key + '" '
          + 'style="background:none;border:none;color:var(--text3);cursor:pointer;padding:2px 6px;font-size:18px;">×</button>'
          + '</div>'
        : '<div class="legal-placeholder-' + key + '" style="font-size:13px;color:var(--text2);">'
          + '<i class="ti ti-upload" style="font-size:20px;display:block;margin-bottom:4px;"></i>'
          + 'PDF oder HTML hierher ziehen oder klicken</div>';
      return '<div style="margin-bottom:12px;">'
        + '<div style="font-size:13px;font-weight:500;margin-bottom:6px;">' + label + '</div>'
        + '<div class="legal-drop-zone" data-key="' + key + '" '
        + 'style="border:2px dashed var(--border2);border-radius:8px;padding:14px 16px;'
        + 'text-align:center;cursor:pointer;transition:border-color .2s;position:relative;">'
        + '<input type="file" accept=".pdf,.html" data-key="' + key + '" '
  ...
  // ── Impressum & Datenschutz Upload-Handler ──────────────────
  function _legalHandleFile(key, file) {
    if (!file) return;
    const reader = new FileReader();
    reader.onload = e => {
      // Store base64 in a temp holder on the drop zone
      const zone = document.querySelector(`.legal-drop-zone[data-key="${key}"]`);
      if (!zone) return;
      zone._pendingData   = e.target.result;   // base64 data URL
      zone._pendingName   = file.name;
      // Update UI
      const placeholder = document.querySelector(`.legal-placeholder-${key}`);
      if (placeholder) placeholder.style.display = 'none';
      let fn = document.querySelector(`.legal-filename-${key}`);
      if (!fn) {
        zone.innerHTML = `
          <div style="display:flex;align-items:center;justify-content:center;gap:8px;font-size:13px;">
            <i class="ti ti-file-filled" style="color:#3b82f6;font-size:18px;"></i>
            <span style="color:var(--text);" class="legal-filename-${key}">${file.name}</span>
            <button type="button" class="legal-remove-btn" data-key="${key}"
              style="background:none;border:none;color:var(--text3);cursor:pointer;padding:2px 4px;font-size:18px;line-height:1;">×</button>
          </div>
          <input type="file" accept=".pdf,.html" data-key="${key}"
            style="position:absolute;inset:0;opacity:0;cursor:pointer;" />`;
        // Re-attach file input listener
        zone.querySelector('input[type="file"]').addEventListener('change', ev => {
          if (ev.target.files[0]) _legalHandleFile(key, ev.target.files[0]);
        });
        // Re-attach remove listener
        zone.querySelector('.legal-remove-btn').addEventListener('click', ev => {
          ev.stopPropagation();
          _legalRemove(key);
        });
      } else {
      ...
      toast(`${file.name} bereit zum Speichern.`);
    };
    reader.readAsDataURL(file);
  }

  function _legalRemove(key) {
    const zone = document.querySelector(`.legal-drop-zone[data-key="${key}"]`);
    if (!zone) return;
    zone._pendingData = '__remove__';
    zone._pendingName = '';
    const label = key === 'impressum' ? 'Impressum' : 'Datenschutzerklärung';
    zone.innerHTML = `
      <input type="file" accept=".pdf,.html" data-key="${key}"
        style="position:absolute;inset:0;opacity:0;cursor:pointer;" />
      <div style="font-size:13px;color:var(--text2);" class="legal-placeholder-${key}">
        <i class="ti ti-upload" style="font-size:20px;display:block;margin-bottom:4px;"></i>
        PDF oder HTML hierher ziehen oder klicken
      </div>`;
    zone.querySelector('input[type="file"]').addEventListener('change', ev => {
      if (ev.target.files[0]) _legalHandleFile(key, ev.target.files[0]);
    });
    _legalAttachDrop(zone, key);
  }

  function _legalAttachDrop(zone, key) {
    zone.addEventListener('dragover', e => { e.preventDefault(); zone.style.borderColor = '#3b82f6'; });
    zone.addEventListener('dragleave', () => { zone.style.borderColor = ''; });
    zone.addEventListener('drop', e => {
      e.preventDefault(); zone.style.borderColor = '';
      const file = e.dataTransfer.files[0];
      if (file) _legalHandleFile(key, file);
    });
  }

  document.querySelectorAll('.legal-drop-zone').forEach(zone => {
    const key = zone.dataset.key;
    // File input change
    zone.querySelector('input[type="file"]')?.addEventListener('change', e => {
      if (e.target.files[0]) _legalHandleFile(key, e.target.files[0]);
    });
    // Remove button
    zone.querySelector('.legal-remove-btn')?.addEventListener('click', e => {
      e.stopPropagation(); _legalRemove(key);
    });
    // Drag & Drop
    _legalAttachDrop(zone, key);
  });
...
function openAltPickerModal({ slot, isPrivate, takenSlots, isStaff = false }) {
  ...
  function rerender() {
    ...
    bindSendButtons();
    // Textarea mit aktuellem Vorlagentext befüllen
  ...
  function buildAltMsg(termineLabels) {
    ...
    if (isStaff) {
      tpl = cfg.altTemplateMitarbeiter ||
        `Hallo {vorname} \xf0\x9f\x91\x8b\n\nder angeforderte Termin ({apptType}) klappt leider nicht. Ich habe f\xc3\xbcr uns folgende freie Zeitfenster:\n\n{termine}\n\nSag mir kurz, was dir passt! \xf0\x9f\x99\x8c\n{gruss}`;
    } else if (isPrivate) {
      tpl = cfg.altTemplatePrivat ||
        `Hey {vorname} \xf0\x9f\x91\x8b\n\nder gew\xc3\xbcnschte Termin klappt leider nicht \xe2\x80\x93 aber ich habe noch folgende freie Zeitfenster:\n\n{termine}\n\nSag mir einfach, was dir passt! \xf0\x9f\x99\x8c\n{gruss}`;
    } else {
      tpl = cfg.altTemplateMandant ||
        `Guten Tag {vorname},\n\nder gew\xc3\xbcnschte Termin ist leider nicht verf\xc3\xbcgbar. Ich biete Ihnen folgende freie Zeitfenster an:\n\n{termine}\n\nBitte teilen Sie mir mit, welcher Termin f\xc3\xbcr Sie passt.\n\nMit freundlichen Gr\xc3\xbc\xc3\x9fen\n{gruss}`;
    }
    return tpl
      .replace(/{vorname}/g,   slot.vorname   || '')
      .replace(/{nachname}/g,  slot.nachname  || '')
      .replace(/{apptType}/g,  slot.apptType  || 'Termin')
      .replace(/{termine}/g,   termineLabels.map(l => '\xf0\x9f\x93\x85 ' + l).join('\n'))
      .replace(/{gruss}/g,     ownerName);
  }

  function sendAlt(via) {
    if (chosen.length === 0) return;
    // Bearbeiteten Text aus Textarea nehmen, falls vorhanden
    const ta = document.getElementById('altPickerMsgPreview');
...
function showPrivateAlternativesModal(slot, freeSlots) {
  ...
  // Vorlage aus Einstellungen laden
  const _buildPrivMsg = (termineLabels) => {
    const cfg = JSON.parse(localStorage.getItem('crmSettings') || '{}');
    const ownerName = cfg.ownerName || '';
    const defTpl = `Hey {vorname} 👋\n\nder gewünschte Termin klappt leider nicht. Ich habe aber noch folgende freie Zeitfenster:\n\n{termine}\n\nSag mir einfach, was dir passt! 🙌\n{gruss}`;
    const tpl = cfg.altTemplatePrivat || defTpl;
    return tpl
      .replace(/{vorname}/g, slot.vorname || '')
      .replace(/{nachname}/g, slot.nachname || '')
  ...
  const updatePrivPreview = () => {
    const checked = [...dlg.querySelectorAll('.alt-slot-check:checked')];
    const ta = document.getElementById('privAltMsgPreview');
    if (ta) ta.value = _buildPrivMsg(checked.map(c => c.dataset.label));
  };

  // Textarea für bearbeitbaren Nachrichtentext
  const privSlotListEl = document.getElementById('altSlotList');
  if (privSlotListEl) {
    const previewWrap = document.createElement('div');
    previewWrap.style.cssText = 'margin-top:14px;margin-bottom:4px;';
    previewWrap.innerHTML = `
      <label style="font-size:12px;color:var(--text3);font-weight:600;display:block;margin-bottom:6px;">
        <i class="ti ti-pencil" style="font-size:11px;"></i> Nachrichtentext (bearbeitbar)
      </label>
      <textarea id="privAltMsgPreview" rows="7"
        style="width:100%;background:var(--bg3);border:1px solid var(--border2);color:var(--text);
        border-radius:8px;padding:10px 12px;font-size:13px;font-family:inherit;resize:vertical;line-height:1.6;">
      </textarea>`;
    privSlotListEl.insertAdjacentElement('afterend', previewWrap);
    updatePrivPreview();
  }
  ...
  document.getElementById('altSendBtn').onclick = () => {
    ...
    const ta = document.getElementById('privAltMsgPreview');
    const msg = ta ? ta.value : _buildPrivMsg(checked.map(c => c.dataset.label));
...
function showBusinessAlternativesModal(slot, freeSlots) {
  ...
  // Vorlage aus Einstellungen laden und Platzhalter befüllen
  const _buildBizMsg = (termineLabels) => {
    const cfg = JSON.parse(localStorage.getItem('crmSettings') || '{}');
    const ownerName = cfg.ownerName || '';
    const defTpl = `Guten Tag {vorname},\n\nder gewünschte Termin ist leider nicht verfügbar. Ich biete Ihnen folgende freie Zeitfenster an:\n\n{termine}\n\nBitte teilen Sie mir mit, welcher Termin für Sie passt.\n\nMit freundlichen Grüßen\n{gruss}`;
    const tpl = cfg.altTemplateMandant || defTpl;
    return tpl
      .replace(/{vorname}/g, slot.vorname || '')
      .replace(/{nachname}/g, slot.nachname || '')
      .replace(/{termine}/g, termineLabels.map(l => '📅 ' + l).join('\n'))
  ...
  // Vorschau-Textarea dynamisch aktualisieren wenn Slots ausgewählt werden
  const updatePreview = () => {
    const checked = [...dlg.querySelectorAll('.biz-alt-slot-check:checked')];
    const labels = checked.map(c => c.dataset.label);
    const ta = document.getElementById('bizAltMsgPreview');
    if (ta) ta.value = _buildBizMsg(labels);
  };

  // Textarea für bearbeitbaren Nachrichtentext einfügen
  const slotListEl = document.getElementById('bizAltSlotList');
  if (slotListEl) {
    const previewWrap = document.createElement('div');
    previewWrap.style.cssText = 'margin-top:14px;margin-bottom:4px;';
    previewWrap.innerHTML = `
      ...
      <textarea id="bizAltMsgPreview" rows="7"
        style="width:100%;background:var(--bg3);border:1px solid var(--border2);color:var(--text);
        border-radius:8px;padding:10px 12px;font-size:13px;font-family:inherit;resize:vertical;line-height:1.6;">
      </textarea>`;
    slotListEl.insertAdjacentElement('afterend', previewWrap);
    updatePreview();
  }
  ...
  const getMsg = () => {
    ...
    const ta = document.getElementById('bizAltMsgPreview');
    return ta ? ta.value : _buildBizMsg(checked.map(c => c.dataset.label));
  };
...
// Stufe 1: Nach 3s Ladetext aktualisieren
const _authWarnTimeout = setTimeout(() => {
  if (_authResolved) return;
  const loadingEl = document.getElementById('auth-loading');
  if (loadingEl) {
    const txt = loadingEl.querySelector('span');
    if (txt) txt.textContent = 'Verbindung wird aufgebaut…';
  }
}, 1500);

// Stufe 2: Nach 6s Login-Screen erzwingen
...
// Stufe 3: Absoluter Notfall-Fallback nach 12s
...
// ============================================================
// 1B. renderContacts OVERRIDE – mit Kategorie-Filter & -Badge
// ============================================================

/**
 * Ersetzt renderContacts so, dass ein dritter Filter-Tab
 * ("Kategorie") mit den drei Unterkategorien hinzukommt.
 * Einfach NACH dem Import der Basisfunktion überschreiben.
 */
const _originalRenderContacts = window.renderContacts;

window.renderContacts = function(filter = '', statusFilter = '', typeFilter = '', catFilter = '') {
  const labels = getCategoryLabels();
  ...
  const filtered = contacts.filter(c => {
    ...
    const matchSearch  = !filter      || search.includes(filter.toLowerCase());
    const matchStatus  = !statusFilter || c.status === statusFilter;
    const matchType    = !typeFilter  || (typeFilter === 'private'
      ? (c.type === 'private' || c.privateContact === true)
      : (c.type !== 'private' && !c.privateContact));
  ...
  function tabStyle(key) {
    ...
    return `
      font-size:13px;padding:6px 14px;border-radius:20px;cursor:pointer;transition:all .15s;
  ...
  document.getElementById('content').innerHTML = `
    <div class="contacts-header">
      ...
      <select class="filter-select" id="statusFilter">
        <option value="">Alle Status</option>
        ${[...new Set(contacts.map(c => c.status).filter(Boolean))].map(s =>
    ...
    <div style="display:flex;gap:8px;margin-bottom:8px;flex-wrap:wrap;">
      <button onclick="renderContacts(document.getElementById('searchInput')?.value||'',document.getElementById('statusFilter')?.value||'','','')"
        style="font-size:13px;padding:6px 14px;border-radius:20px;border:1.5px solid ${!typeFilter&&!catFilter?'var(--accent)':'var(--border2)'};
        background:${!typeFilter&&!catFilter?'rgba(59,130,246,0.12)':'transparent'};color:${!typeFilter&&!catFilter?'var(--accent)':'var(--text2)'};
        cursor:pointer;font-weight:${!typeFilter&&!catFilter?'600':'400'};transition:all .15s;">
        Alle <span style="font-size:11px;opacity:.7;">(${contacts.length})</span>
      </button>
      <button onclick="renderContacts(document.getElementById('searchInput')?.value||'',document.getElementById('statusFilter')?.value||'','business','')"
        style="font-size:13px;padding:6px 14px;border-radius:20px;border:1.5px solid ${typeFilter==='business'&&!catFilter?'var(--purple)':'var(--border2)'};
        background:${typeFilter==='business'&&!catFilter?'rgba(139,92,246,0.12)':'transparent'};color:${typeFilter==='business'&&!catFilter?'var(--purple)':'var(--text2)'};
        cursor:pointer;font-weight:${typeFilter==='business'&&!catFilter?'600':'400'};transition:all .15s;">
        <i class="ti ti-briefcase" style="font-size:12px;"></i> Mandanten <span style="font-size:11px;opacity:.7;">(${businessCount})</span>
      </button>
      <button onclick="renderContacts(document.getElementById('searchInput')?.value||'',document.getElementById('statusFilter')?.value||'','private','')"
        style="font-size:13px;padding:6px 14px;border-radius:20px;border:1.5px solid ${typeFilter==='private'&&!catFilter?'var(--amber)':'var(--border2)'};
        background:${typeFilter==='private'&&!catFilter?'rgba(245,158,11,0.12)':'transparent'};color:${typeFilter==='private'&&!catFilter?'var(--amber)':'var(--text2)'};
        cursor:pointer;font-weight:${typeFilter==='private'&&!catFilter?'600':'400'};transition:all .15s;">
        🔒 Privat <span style="font-size:11px;opacity:.7;">(${privateCount})</span>
      </button>
    </div>
    ...
    <div style="display:flex;gap:6px;margin-bottom:14px;flex-wrap:wrap;align-items:center;">
      <span style="font-size:11px;color:var(--text3);margin-right:2px;text-transform:uppercase;letter-spacing:.05em;">Kategorie:</span>
      ${Object.entries({
        mandant: labels.mandant,
        privat: labels.privat,
        mitarbeiter:   labels.mitarbeiter,
        mandantMitarbeiter: labels.mandantMitarbeiter,
      }).map(([key, lbl]) => `
        <button onclick="renderContacts(document.getElementById('searchInput')?.value||'',document.getElementById('statusFilter')?.value||'','','${key}')"
          style="${tabStyle(key)}">
          ${lbl} <span style="font-size:11px;opacity:.7;">(${catCounts[key]})</span>
        </button>
      `).join('')}
      ${catFilter ? `<button onclick="renderContacts(document.getElementById('searchInput')?.value||'',document.getElementById('statusFilter')?.value||'','','')"
        ...
        <i class="ti ti-x"></i> Filter aufheben</button>` : ''}
    </div>
      ...
      <table class="contacts-table">
        ...
        <tbody>
          ${filtered.map(c => {
            ...
            <tr style="${isPrivate ? 'background:rgba(245,158,11,0.04);' : ''}">
              <td>
                <div class="contact-name" style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;">
                  ${isPrivate ? '<span title="Privater Kontakt" style="font-size:11px;background:rgba(245,158,11,0.15);color:var(--amber);border:1px solid rgba(245,158,11,0.3);border-radius:4px;padding:1px 5px;font-weight:600;flex-shrink:0;">🔒 Privat</span>' : ''}
                  ${c.vorname} ${c.nachname}
                </div>
                <div class="contact-sub">${c.ort || ''}</div>
              ...
              <td>${statusBadge(c.status)}</td>
              <td>
                ${categoryBadge(c.contactCategory)}
                ${auth.currentUser?.email === ALLOWED_EMAIL ? `
                <select onchange="setContactCategory('${c.id}',this.value)"
                  style="margin-top:4px;font-size:11px;background:var(--bg2);border:1px solid var(--border2);color:var(--text3);border-radius:6px;padding:2px 5px;cursor:pointer;max-width:150px;">
  ...
  document.getElementById('searchInput').addEventListener('input', e =>
    renderContacts(e.target.value, document.getElementById('statusFilter').value, typeFilter, catFilter));
  document.getElementById('statusFilter').addEventListener('change', e =>
    renderContacts(document.getElementById('searchInput').value, e.target.value, typeFilter, catFilter));
};
...
function buildAltTemplateSettingsHTML(s) {
  const defMandant = `Guten Tag {vorname},\n\nder von Ihnen gewünschte Termin ist leider nicht verfügbar. Ich biete Ihnen gerne folgende Alternativtermine an:\n\n{termine}\n\nBitte teilen Sie mir mit, welcher Termin für Sie passt – ich freue mich auf unser Gespräch.\n\nMit freundlichen Grüßen\n{gruss}`;
  const defPrivat  = `Hey {vorname} 👋\n\nder gewünschte Termin klappt leider nicht. Ich habe aber noch folgende freie Zeitfenster:\n\n{termine}\n\nSag mir einfach, was dir passt! 🙌\n{gruss}`;
  ...
  return `
    ...
    <div class="settings-section" id="altTemplateSettingsSection">
      ...
      <p style="font-size:13px;color:var(--text2);margin-bottom:6px;line-height:1.5;">
        Diese Texte werden vorausgefüllt, wenn du bei einer Anfrage auf <strong>„Alternativen anbieten"</strong> klickst.
        Du kannst den Text vor dem Senden noch anpassen.
      </p>
      <p style="font-size:12px;color:var(--text3);margin-bottom:16px;">
        Verfügbare Platzhalter: <code style="background:var(--bg3);padding:1px 5px;border-radius:4px;">{vorname}</code>
        <code style="background:var(--bg3);padding:1px 5px;border-radius:4px;">{nachname}</code>
        <code style="background:var(--bg3);padding:1px 5px;border-radius:4px;">{termine}</code> (wird automatisch ersetzt)
      ...
      <div style="display:flex;flex-direction:column;gap:16px;">
        <div class="field full">
          <label style="display:flex;align-items:center;gap:6px;">
            ...
            Vorlage für <strong>Mandanten</strong> (formell, E-Mail &amp; WhatsApp)
          </label>
          <textarea id="s_altTemplateMandant" rows="6"
            ...
            placeholder="Vorlage für Mandanten...">${s.altTemplateMandant || defMandant}</textarea>
        </div>
        <div class="field full">
          <label style="display:flex;align-items:center;gap:6px;">
            <span style="width:10px;height:10px;border-radius:3px;background:#f59e0b;display:inline-block;"></span>
            Vorlage für <strong>Private Kontakte</strong> (per du, WhatsApp)
          </label>
          <textarea id="s_altTemplatePrivat" rows="6"
            style="background:var(--bg2);border:1px solid var(--border2);color:var(--text);border-radius:8px;
            padding:10px 12px;font-size:13px;font-family:inherit;width:100%;resize:vertical;line-height:1.6;"
            placeholder="Vorlage für private Kontakte...">${s.altTemplatePrivat || defPrivat}</textarea>
        </div>
        <div class="field full">
          <label style="display:flex;align-items:center;gap:6px;">
            <span style="width:10px;height:10px;border-radius:3px;background:#8b5cf6;display:inline-block;"></span>
            Vorlage für <strong>Mitarbeiter-Anfragen</strong>
          </label>
          <textarea id="s_altTemplateMitarbeiter" rows="6"
            style="background:var(--bg2);border:1px solid var(--border2);color:var(--text);border-radius:8px;
            padding:10px 12px;font-size:13px;font-family:inherit;width:100%;resize:vertical;line-height:1.6;"
            placeholder="Vorlage für Mitarbeiter-Anfragen...">${s.altTemplateMitarbeiter || defMitarbeiter}</textarea>
        </div>
      </div>
      <p style="font-size:12px;color:var(--text3);margin-top:10px;">
        <i class="ti ti-info-circle"></i>
        Die Vorlagen werden lokal gespeichert und erscheinen vorausgefüllt im Sende-Dialog. Du kannst sie jederzeit hier anpassen.
      </p>
    </div>
  `;
}
...
function _extRenderSettings() {
  // Originale Funktion aufrufen (befüllt #content)
  _origRenderSettings();

  // Abwarten bis DOM gerendert ist, dann Sektion injizieren
  requestAnimationFrame(() => {
    const s = JSON.parse(localStorage.getItem('crmSettings') || '{}');

    // Sektionen vor dem Speichern-Button einfügen
    const saveBtn = document.getElementById('saveSettings');
    if (saveBtn && !document.getElementById('altTemplateSettingsSection')) {
      saveBtn.parentElement.insertAdjacentHTML('beforebegin', buildAltTemplateSettingsHTML(s));
    }
    if (saveBtn && !document.getElementById('categorySettingsSection')) {
      saveBtn.parentElement.insertAdjacentHTML('beforebegin', buildCategorySettingsHTML(s));
    }

    // Speichern-Button: alle Felder mitspeichern
    const originalSaveClick = saveBtn?._catPatched;
    if (saveBtn && !originalSaveClick) {
      ...
      saveBtn.addEventListener('click', () => {
        ...
        const tMandant     = document.getElementById('s_altTemplateMandant');
        const tPrivat      = document.getElementById('s_altTemplatePrivat');
        const tMitarbeiter = document.getElementById('s_altTemplateMitarbeiter');
        if (tMandant)     existing.altTemplateMandant     = tMandant.value;
        if (tPrivat)      existing.altTemplatePrivat      = tPrivat.value;
        if (tMitarbeiter) existing.altTemplateMitarbeiter = tMitarbeiter.value;
        localStorage.setItem('crmSettings', JSON.stringify(existing));
      }, { capture: false });
    }
  });
