...
function businessInboxCount() {
  ...
  return callSlots.filter(s => s.status === 'anfrage' || s.type === 'anfrage').length;
}
            ...
            ">
              ...
              ${slotsHere.map(sl => {
                ...
                const name = isPrivate
                  ...
                  : (c ? `${c.vorname} ${c.nachname}` : '?');
                const color = isPrivate ? '#f59e0b' : (sl.apptColor || (sl.type === 'flex' ? '#f59e0b' : '#3b82f6'));
                const slDate = new Date(sl.datetime);
                const slMin = slDate.getHours() * 60 + slDate.getMinutes();
                ...
                " onmouseover="this.style.opacity='.85';${!isPrivate ? `this.querySelector('.slot-del')&&(this.querySelector('.slot-del').style.display='flex')` : ''}" onmouseout="this.style.opacity='1';${!isPrivate ? `this.querySelector('.slot-del')&&(this.querySelector('.slot-del').style.display='none')` : ''}">
                  <div ${!isPrivate ? `onclick="showContact('${sl.contactId||''}')"` : ''} style="min-width:0;">
                    <div style="font-size:11px;font-weight:700;color:${color};white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">
                      ${isPrivate ? '🔒 ' : ''}${new Date(sl.datetime).toLocaleTimeString('de-DE',{hour:'2-digit',minute:'2-digit'})}${sl.apptType ? ' · ' + sl.apptType : ''}
                    </div>
...
{signatur}`;
  ...
  document.getElementById('content').innerHTML = `
    ...
    <!-- ── TERMINBESTÄTIGUNG ── -->
    <div class="settings-section">
      <h3>📧 Terminbestätigung</h3>
      <p style="font-size:13px;color:var(--text2);margin-bottom:14px;">
        Wird sofort nach Bestätigung eines Termins versendet – inklusive .ics-Kalenderdatei im Anhang.<br/>
      ...
      <div class="form-grid">
        <div class="field full">
          ...
          <select id="s_confirmTiming" style="background:var(--bg3);border:1px solid var(--border2);color:var(--text);border-radius:8px;padding:9px 12px;font-size:13px;outline:none;font-family:inherit;">
            <option value="immediately" ${(s.confirmTiming||'immediately')==='immediately'?'selected':''}>Sofort nach Bestätigung</option>
            <option value="1day" ${s.confirmTiming==='1day'?'selected':''}>1 Tag vor dem Termin</option>
            <option value="2days" ${s.confirmTiming==='2days'?'selected':''}>2 Tage vor dem Termin</option>
            <option value="3days" ${s.confirmTiming==='3days'?'selected':''}>3 Tage vor dem Termin</option>
          </select>
        </div>
    ...
    <div class="settings-section">
      <h3>📲 Terminbenachrichtigung</h3>
      <p style="font-size:13px;color:var(--text2);margin-bottom:14px;">
        Eine separate Info-E-Mail mit allen Termindetails – ohne Kalenderanhang. Nützlich als zusätzliche Erinnerung.<br/>
        <span style="font-size:12px;color:var(--text3);">Platzhalter: <code style="background:var(--bg3);padding:1px 5px;border-radius:4px;">{vorname}</code> <code style="background:var(--bg3);padding:1px 5px;border-radius:4px;">{nachname}</code> <code style="background:var(--bg3);padding:1px 5px;border-radius:4px;">{datum}</code> <code style="background:var(--bg3);padding:1px 5px;border-radius:4px;">{uhrzeit}</code> <code style="background:var(--bg3);padding:1px 5px;border-radius:4px;">{signatur}</code></span>
      </p>
      <div class="form-grid">
        <div class="field full">
          <label>Versandzeitpunkt</label>
          <select id="s_notifyTiming" style="background:var(--bg3);border:1px solid var(--border2);color:var(--text);border-radius:8px;padding:9px 12px;font-size:13px;outline:none;font-family:inherit;">
            <option value="immediately" ${(s.notifyTiming||'immediately')==='immediately'?'selected':''}>Sofort nach Bestätigung</option>
            <option value="1day" ${s.notifyTiming==='1day'?'selected':''}>1 Tag vor dem Termin</option>
            <option value="2days" ${s.notifyTiming==='2days'?'selected':''}>2 Tage vor dem Termin</option>
            <option value="3days" ${s.notifyTiming==='3days'?'selected':''}>3 Tage vor dem Termin</option>
          </select>
        </div>
        <div class="field full"><label>Betreff</label>
    ...
    <div class="settings-section">
      ...
      <p style="font-size:13px;color:var(--text2);margin-bottom:14px;">
        Erinnerungs-E-Mail die automatisch kurz vor dem Termin versendet wird.<br/>
        <span style="font-size:12px;color:var(--text3);">Platzhalter: <code style="background:var(--bg3);padding:1px 5px;border-radius:4px;">{vorname}</code> <code style="background:var(--bg3);padding:1px 5px;border-radius:4px;">{nachname}</code> <code style="background:var(--bg3);padding:1px 5px;border-radius:4px;">{datum}</code> <code style="background:var(--bg3);padding:1px 5px;border-radius:4px;">{uhrzeit}</code> <code style="background:var(--bg3);padding:1px 5px;border-radius:4px;">{signatur}</code></span>
      </p>
      <div class="form-grid">
        <div class="field full">
          <label>Versandzeitpunkt</label>
          <select id="s_reminderTiming" style="background:var(--bg3);border:1px solid var(--border2);color:var(--text);border-radius:8px;padding:9px 12px;font-size:13px;outline:none;font-family:inherit;">
            <option value="sameday_09" ${(s.reminderTiming||'sameday_09')==='sameday_09'?'selected':''}>Am Termintag um 09:00 Uhr</option>
            <option value="sameday_08" ${s.reminderTiming==='sameday_08'?'selected':''}>Am Termintag um 08:00 Uhr</option>
            <option value="1day" ${s.reminderTiming==='1day'?'selected':''}>1 Tag vorher (09:00 Uhr)</option>
            <option value="2days" ${s.reminderTiming==='2days'?'selected':''}>2 Tage vorher (09:00 Uhr)</option>
            <option value="3days" ${s.reminderTiming==='3days'?'selected':''}>3 Tage vorher (09:00 Uhr)</option>
          </select>
        </div>
        <div class="field full"><label>Betreff</label>
  ...
  document.getElementById('saveSettings').addEventListener('click', () => {
    ...
    const settings = {
      ...
      confirmTiming:      document.getElementById('s_confirmTiming')?.value || 'immediately',
      ...
      notifyTiming:       document.getElementById('s_notifyTiming')?.value || 'immediately',
      ...
      reminderTiming:     document.getElementById('s_reminderTiming')?.value || 'sameday_09',
      dayConfigs,
      availDays,
      availStart,
      availEnd,
        ...
        <label style="display:flex;align-items:flex-start;gap:10px;padding:10px 12px;background:var(--bg3);border:1px solid var(--border2);border-radius:8px;cursor:pointer;transition:background .15s;"
          ...
          <div>
            ...
            <div style="font-size:11px;color:var(--text3);margin-top:2px;">
              Versand: ${(()=>{const t=(JSON.parse(localStorage.getItem('crmSettings')||'{}').confirmTiming||'immediately');return{immediately:'Sofort nach Bestätigung','1day':'1 Tag vor dem Termin','2days':'2 Tage vor dem Termin','3days':'3 Tage vor dem Termin'}[t]||'Sofort nach Bestätigung';})()}
            </div>
          </div>
        </label>
        <label style="display:flex;align-items:flex-start;gap:10px;padding:10px 12px;background:var(--bg3);border:1px solid var(--border2);border-radius:8px;cursor:pointer;transition:background .15s;"
          ...
          <div>
            <div style="font-size:14px;font-weight:500;">🔔 Terminerinnerung</div>
            <div style="font-size:11px;color:var(--text3);margin-top:2px;">
              Versand: ${(()=>{const t=(JSON.parse(localStorage.getItem('crmSettings')||'{}').reminderTiming||'sameday_09');return{sameday_09:'Am Termintag um 09:00 Uhr',sameday_08:'Am Termintag um 08:00 Uhr','1day':'1 Tag vorher (09:00 Uhr)','2days':'2 Tage vorher (09:00 Uhr)','3days':'3 Tage vorher (09:00 Uhr)'}[t]||'Am Termintag um 09:00 Uhr';})()}
            </div>
          </div>
        </label>
        <label style="display:flex;align-items:flex-start;gap:10px;padding:10px 12px;background:var(--bg3);border:1px solid var(--border2);border-radius:8px;cursor:pointer;transition:background .15s;"
          ...
          <div>
            <div style="font-size:14px;font-weight:500;">📲 Terminbenachrichtigung</div>
            <div style="font-size:11px;color:var(--text3);margin-top:2px;">
              Versand: ${(()=>{const t=(JSON.parse(localStorage.getItem('crmSettings')||'{}').notifyTiming||'immediately');return{immediately:'Sofort nach Bestätigung','1day':'1 Tag vor dem Termin','2days':'2 Tage vor dem Termin','3days':'3 Tage vor dem Termin'}[t]||'Sofort nach Bestätigung';})()}
            </div>
          </div>
        </label>
      </div>
